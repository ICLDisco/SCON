...
AC_DEFUN([SCON_LOAD_PLATFORM], [
    ...
    if test "$with_platform" = "yes" ; then
        ...
        SCON_LOG_MSG([Loading environment file $with_platform, with contents below])
        SCON_LOG_FILE([$with_platform])
        ...
        if test "$SCON_PLATFORM_LOADED" != "" ; then
           platform_loaded="$SCON_PLATFORM_LOADED"
        else
           platform_loaded="$with_platform"
        fi
        echo "Loaded platform arguments for $platform_loaded"
        SCON_LOG_MSG([Loaded platform arguments for $platform_loaded])
