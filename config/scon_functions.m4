...
SCON_CONFIGURE_USER="`whoami`"
SCON_CONFIGURE_HOST="`hostname | head -n 1`"
SCON_CONFIGURE_DATE="`date`"

#
# Save these details so that they can be used in scon_info later
#
AC_SUBST(SCON_CONFIGURE_USER)
AC_SUBST(SCON_CONFIGURE_HOST)
AC_SUBST(SCON_CONFIGURE_DATE)])dnl

dnl #######################################################################
dnl #######################################################################
dnl #######################################################################

AC_DEFUN([SCON_BASIC_SETUP],[
#
# Save some stats about this build
#
...
#
# See if we can find an old installation of SCON to overwrite
#

# Stupid autoconf 2.54 has a bug in AC_PREFIX_PROGRAM -- if scon_clean
# is not found in the path and the user did not specify --prefix,
# we'll get a $prefix of "."

scon_prefix_save="$prefix"
AC_PREFIX_PROGRAM(scon_clean)
if test "$prefix" = "."; then
    prefix="$scon_prefix_save"
fi
unset scon_prefix_save
...
if test "$program_prefix$program_suffix$program_transform_name" != "NONENONEs,x,x," ; then
    AC_MSG_WARN([*** The SCON configure script does not support --program-prefix, --program-suffix or --program-transform-name. Users are recommended to instead use --prefix with a unique directory and make symbolic links as desired for renaming.])
...
for val in ${$1}; do
    ...
    scon_done="`expr $scon_i \> $scon_count`"
    while test "$scon_found" = "0" && test "$scon_done" = "0"; do

    # Have we seen this token already?  Prefix the comparison with
    # "x" so that "-Lfoo" values won't be cause an error.

    scon_eval="expr x$val = x\$scon_array_$scon_i"
    scon_found=`eval $scon_eval`
    ...
    -Xclang)
            scon_found=0
            scon_i=`expr $scon_count + 1`
            ;;
    esac

    # If we didn't find the token, add it to the "array"

    if test "$scon_found" = "0"; then
    scon_eval="scon_array_$scon_i=$val"
    eval $scon_eval
    scon_count="`expr $scon_count + 1`"
    else
    scon_i="`expr $scon_i - 1`"
    fi
done
...
scon_done="`expr $scon_i \> $scon_count`"
scon_newval=
while test "$scon_done" = "0"; do
    scon_eval="scon_newval=\"$scon_newval \$scon_array_$scon_i\""
    eval $scon_eval

    scon_eval="unset scon_array_$scon_i"
    eval $scon_eval
...
scon_newval="`echo $scon_newval`"
scon_eval="$scon_name=\"$scon_newval\""
eval $scon_eval

# Clean up

unset scon_name scon_i scon_done scon_newval scon_eval scon_count])dnl

dnl #######################################################################
dnl #######################################################################
dnl #######################################################################

# SCON_APPEND_UNIQ(variable, new_argument)
# ----------------------------------------
# Append new_argument to variable if not already in variable.  This assumes a
# space seperated list.
#
# This could probably be made more efficient :(.
AC_DEFUN([SCON_APPEND_UNIQ], [
for arg in $2; do
    scon_found=0;
    for val in ${$1}; do
...
# SCON_FLAGS_APPEND_UNIQ(variable, new_argument)
# ----------------------------------------------
...
AC_DEFUN([SCON_FLAGS_APPEND_UNIQ], [
    SCON_VAR_SCOPE_PUSH([scon_tmp scon_append])

    for arg in $2; do
        scon_tmp=`echo $arg | cut -c1-2`
        scon_append=1
        AS_IF([test "$scon_tmp" = "-I" || test "$scon_tmp" = "-L" || test "$scon_tmp" = "-l"],
              [for val in ${$1}; do
                   AS_IF([test "x$val" = "x$arg"], [scon_append=0])
               done])
        AS_IF([test "$scon_append" = "1"],
...
SCON_VAR_SCOPE_PUSH([scon_prog scon_file scon_dir scon_sentinel])

scon_prog=$1

IFS_SAVE=$IFS
IFS="$PATH_SEPARATOR"
for scon_dir in $PATH; do
    if test -x "$scon_dir/$scon_prog"; then
        $2="$scon_dir/$scon_prog"
        break
    fi
done
IFS=$IFS_SAVE
...
# Declare some variables; use SCON_VAR_SCOPE_END to ensure that they
# are cleaned up / undefined.
AC_DEFUN([SCON_VAR_SCOPE_PUSH],[

    # Is the private index set?  If not, set it.
    if test "x$scon_scope_index" = "x"; then
        scon_scope_index=1
    fi
    ...
    for scon_var in $1; do
        scon_str="scon_str=\"\$$scon_var\""
        eval $scon_str

        if test "x$scon_str" != "x"; then
            AC_MSG_WARN([Found configure shell variable clash!])
            AC_MSG_WARN([[SCON_VAR_SCOPE_PUSH] called on "$scon_var",])
            AC_MSG_WARN([but it is already defined with value "$scon_str"])
    ...
    scon_str="scon_scope_$scon_scope_index=\"$1\""
    eval $scon_str
    unset scon_str

    env | grep scon_scope
    scon_scope_index=`expr $scon_scope_index + 1`
])dnl

# Unset a bunch of variables that were previously set
AC_DEFUN([SCON_VAR_SCOPE_POP],[
    # Unwind the index
    scon_scope_index=`expr $scon_scope_index - 1`
    scon_scope_test=`expr $scon_scope_index \> 0`
    if test "$scon_scope_test" = "0"; then
        AC_MSG_WARN([[SCON_VAR_SCOPE_POP] popped too many SCON configure scopes.])
        AC_MSG_WARN([This usually indicates an error in configure.])
        AC_MSG_ERROR([Cannot continue])
    fi

    # Get the variable names from that index
    scon_str="scon_str=\"\$scon_scope_$scon_scope_index\""
    eval $scon_str

    # Iterate over all the variables and unset them all
    for scon_var in $scon_str; do
        unset $scon_var
    done
])dnl
...
#
# SCON_WITH_OPTION_MIN_MAX_VALUE(NAME,DEFAULT_VALUE,LOWER_BOUND,UPPER_BOUND)
# Defines a variable SCON_MAX_xxx, with "xxx" being specified as parameter $1 as "variable_name".
...
#
AC_DEFUN([SCON_WITH_OPTION_MIN_MAX_VALUE], [
    max_value=[$2]
    ...
    AC_DEFINE_UNQUOTED([SCON_MAX_]m4_toupper($1), $max_value,
                       [Maximum length of ]m4_translit($1, [_], [ ])[s (default is $2)])
    [SCON_MAX_]m4_toupper($1)=$max_value
    AC_SUBST([SCON_MAX_]m4_toupper($1))
])dnl
...
# Usage: SCON_COMPUTE_MAX_VALUE(number_bytes, variable_to_set, action if overflow)
# Compute maximum value of datatype of
# number_bytes, setting the result in the second argument.  Assumes a
# signed datatype.
AC_DEFUN([SCON_COMPUTE_MAX_VALUE], [
    ...
    scon_num_bits=`expr $1 \* 8 - 1`
    newval=1
    value=1
    overflow=0

    while test $scon_num_bits -ne 0 ; do
        newval=`expr $value \* 2`
        if test 0 -eq `expr $newval \< 0` ; then
            # if the new value is not negative, next iteration...
            value=$newval
            scon_num_bits=`expr $scon_num_bits - 1`
            ...
            # below...)
            if test 0 -eq $scon_num_bits ; then
                value=`expr $value - 1`
            fi
        else
            ...
            if test 0 -eq `expr $newval \< 0` ; then
                ...
                # scon_num_bits is 1, we didn't actually overflow.
                # Otherwise, we overflowed.
                if test 1 -ne $scon_num_bits ; then
                    overflow=1
                fi
            else
