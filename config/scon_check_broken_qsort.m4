...
dnl the scon_qsort instead.

# check for broken qsort
# SCON_CHECK_BROKEN_QSORT(prefix, [action-if-found], [action-if-not-found])
# --------------------------------------------------------
AC_DEFUN([SCON_CHECK_BROKEN_QSORT],[
  ...
  AC_DEFINE_UNQUOTED([SCON_HAVE_BROKEN_QSORT], [$define_result],
