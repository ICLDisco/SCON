...
# Minimum versions
my $scon_automake_version = "1.12.2";
my $scon_autoconf_version = "2.69";
my $scon_libtool_version = "2.4.2";

# Search paths
my $scon_autoconf_search = "autoconf";
my $scon_automake_search = "automake";
my $scon_libtoolize_search = "libtoolize;glibtoolize";
...
sub ignored {
    ...
    # If this directory does not have .scon_ignore, or if it has a
    # .scon_unignore that has my username in it, then add it to the
    # list of components.
    my $ignored = 0;

    if (-f "$dir/.scon_ignore") {
        $ignored = 1;
    }
    if (-f "$dir/.scon_unignore") {
        open(UNIGNORE, "$dir/.scon_unignore") ||
            my_die "Can't open $dir/.scon_unignore file";
        my $unignore;
        $unignore .= $_
            while (<UNIGNORE>);
        close(UNIGNORE);
    ...
    if (exists($exclude_list->{$framework}) &&
        ...
        if (-d $dir) {
            ...
            foreach my $d (readdir(DIR)) {
                ...
                verbose "--- Found scon / $framework / $d component: src/mca/$framework/$d\n";
                ...
                # Check ignore status
                if (ignored("$dir/$d")) {
                    verbose "    => Ignored (found .scon_ignore file)\n";
                } else {
...
sub mca_generate_framework_header(\$\@) {
    ...
    foreach my $framework (@frameworks) {
        ...
        if ($framework ne "common") {
            my $framework_name = "scon_${framework}_base_framework";
            $framework_array_output .= "    &$framework_name,\n";
            $framework_decl_output .= "extern scon_mca_base_framework_t $framework_name;\n";
        }
    }

    my $ifdef_string = uc "scon_FRAMEWORKS_H";
...
sub mca_process_project {
    ...
    foreach my $d (@my_dirs) {
        ...
        if ("common" eq $d ||
            ...
            verbose "\n=== Found scon framework: src/mca/$d\n";
...
dnl Frameworks in the scon project and their corresponding directories
m4_define([mca_scon_framework_list], [$frameworks_comma])
...
";
    ...
    foreach my $f (@mykeys) {
        ...
        $m4 .= "dnl Components in the scon / $f framework
m4_define([mca_scon_${f}_m4_config_component_list], [$m4_config_component_list])
m4_define([mca_scon_${f}_no_config_component_list], [$no_config_component_list])
...
tools:
    ...
    GNU Autoconf: $scon_autoconf_version
    GNU Automake: $scon_automake_version
    GNU Libtool: $scon_libtool_version
=================================================================\n";
    my_exit(1);
}
...
# Verify that we're in the SCON root directory by checking for a token file.

my_die "Not at the root directory of an SCON source tree"
    if (! -f "config/scon_mca.m4");

my_die "autogen.pl has been invoked in the source tree of a SCON distribution tarball; aborting...
...
my $step = 1;
verbose "SCON autogen (buckle up!)

$step. Checking tool versions\n\n";

# Check the autotools revision levels
&find_and_check("autoconf", $scon_autoconf_search, $scon_autoconf_version);
&find_and_check("libtool", $scon_libtoolize_search, $scon_libtool_version);
&find_and_check("automake", $scon_automake_search, $scon_automake_version);
...
# Figure out if we're at the top level of the SCON tree or not.
if (! (-f "VERSION" && -f "configure.ac" && -f $topdir_file)) {
    print("\n\nYou must run this script from the top-level directory of the SCON tree.\n\n");
    my_exit(1);
}
...
verbose "
================================================
SCON autogen: completed successfully.  w00t!
================================================\n\n";
