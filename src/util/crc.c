...
#if (SCON_ALIGNMENT_LONG == 8)
#define SCON_CRC_WORD_MASK_ 0x7
#elif (SCON_ALIGNMENT_LONG == 4)
#define SCON_CRC_WORD_MASK_ 0x3
#else
#define SCON_CRC_WORD_MASK_ 0xFFFF
#endif


#define WORDALIGNED(v) \
    (((intptr_t)v & SCON_CRC_WORD_MASK_) ? false : true)
...
static bool _scon_crc_table_initialized = false;
static unsigned int _scon_crc_table[256];
...
void scon_initialize_crc_table(void)
{
    ...
    for (i = 0; i < 256; i++) {
        ...
        _scon_crc_table[i] = crc_accum;
    }

    /* set global bool to true to do this work once! */
    _scon_crc_table_initialized = true;
    return;
}

unsigned int scon_bcopy_uicrc_partial(
    const void *  source,
    void *  destination,
    size_t copylen,
    size_t crclen,
    unsigned int partial_crc)
{
    ...
    if (!_scon_crc_table_initialized) {
        ...
        while (copylen >= sizeof(unsigned int)) {
            ...
            *dst++ = tmp;
            ...
            for (j = 0; j < (int)sizeof(unsigned int); j++) {
                ...
                partial_crc = (partial_crc << 8) ^ _scon_crc_table[i];
            }
...
unsigned int scon_uicrc_partial(
