...
#define SCON_DEFAULT_TMPDIR "/tmp"

/*
 * Merge two environ-like char arrays, ensuring that there are no
 * duplicate entires
 */
char **scon_environ_merge(char **minor, char **major)
{
    int i;
    char **ret = NULL;
    char *name, *value;
    ...
    if (NULL == major) {
        if (NULL == minor) {
            ...
            return scon_argv_copy(minor);
        }
    }

    /* First, copy major */

    ret = scon_argv_copy(major);
    ...
    /* Now go through minor and call scon_setenv(), but with overwrite
       as false */

    for (i = 0; NULL != minor[i]; ++i) {
        value = strchr(minor[i], '=');
        if (NULL == value) {
            scon_setenv(minor[i], NULL, false, &ret);
        } else {
            ...
            *value = '\0';
            scon_setenv(name, value + 1, false, &ret);
            free(name);
        }
    }
 ...
 * array
 */
 scon_status_t scon_setenv(const char *name, const char *value, bool overwrite,
                char ***env)
{
    int i;
    ...
    if( NULL == env ) {
        ...
        i = 0;
        scon_argv_append(&i, env, newvalue);
        free(newvalue);
        return SCON_SUCCESS;
    }
    ...
    if( *env == environ ) {
        /* THIS IS POTENTIALLY A MEMORY LEAK!  But I am doing it
           ...
           astonishmet for SCON developers (i.e., those that don't
           check the return code of scon_setenv() and notice that we
 ...
 * environ-like array
 */
 scon_status_t scon_unsetenv(const char *name, char ***env)
{
    int i;
    char *compare;
    size_t len;
    ...
    return (found) ? SCON_SUCCESS : SCON_ERR_NOT_FOUND;
}
