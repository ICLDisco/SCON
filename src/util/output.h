...
/** @file
 * SCON output stream facility.
 *
 * The SCON output stream facility is used to send output from the SCON
 ...
 * scon_output_open() function call, and then sent output via
 * scon_output_verbose(), SCON_OUTPUT(), and scon_output().  Streams are
 * closed with scon_output_close().
 *
 ...
 * - file
 *
 * Which outputs to use are specified during scon_output_open().
 *
 ...
 * example).  Most MCA components and internals of SCON won't be
 * affected by this, but some RTE / startup aspects of SCON will
 * not be able to write to a file for output.  See scon_output() for
 * details on what happens in these cases.
 *
 * scon_output_open() returns an integer handle that is used in
 * successive calls to SCON_OUTPUT() and scon_output() to send output to
 * the stream.
 *
 * The default "verbose" stream is opened after invoking
 * scon_output_init() (and closed after invoking
 * scon_output_finalize()).  This stream outputs to stderr only, and
...
#ifndef SCON_OUTPUT_H_
#define SCON_OUTPUT_H_
 ...
 * output (both from scon_output and stdout/stderr of processes) to
 * syslog:
 *
 * SCON_OUTPUT_REDIRECT - set to "syslog" to redirect to syslog. Other
 *                        options may someday be supported
 * SCON_OUTPUT_SYSLOG_PRI - set to "info", "error", or "warn" to have
 *                        output sent to syslog at that priority
 * SCON_OUTPUT_SYSLOG_IDENT - a string identifier for the log
 *
 ...
 */
extern bool scon_output_redirected_to_syslog;
extern int scon_output_redirected_syslog_pri;

/**
 * \class scon_output_stream_t
 *
 * Structure used to request the opening of a SCON output stream.  A
 * pointer to this structure is passed to scon_output_open() to tell
 * the scon_output subsystem where to send output for a given stream.
 ...
 * destinations through the scon_output facility.
 *
 ...
 * from scon_output_open().
 */
struct scon_output_stream_t {
    /** Class parent */
    scon_object_t super;
     ...
     * scon_output_verbose() function.  Verbose levels are ignored in
     * SCON_OUTPUT() and scon_output().
     *
    ...
    /**
     * When scon_output_stream_t::lds_want_syslog is true, this field is
     ...
     *
     * If a NULL value is given, the string "scon" is used.
     */
     ...
     * to an internal structure in the call to scon_output_open()!
     */
    char *lds_prefix;
     ...
     * the stream unless SCON was configured with --enable-debug.
     */
    ...
    bool lds_want_file;
    /**
     * When scon_output_stream_t::lds_want_file is true, this field
     ...
     * prefix set by scon_output_set_output_file_info() (e.g.,
     ...
     * exist when scon_output_open() is invoked.  See scon_output()
     ...
     * Convenience typedef
     */
    typedef struct scon_output_stream_t scon_output_stream_t;
     ...
     * stream.
     */
    void scon_output_finalize(void);

    /**
     * Opens an output stream.
     *
     * @param lds A pointer to scon_output_stream_t describing what the
     ...
     * using it in successive calls to SCON_OUTPUT(), scon_output(),
     * scon_output_switch(), and scon_output_close().
     *
     ...
     * unspecified manner.
     *
     * Be sure to see SCON_output() for a description of what happens
     * when open_open() / scon_output() is directed to send output to a
     * file but the process session directory does not yet exist.
     */
    int scon_output_open(scon_output_stream_t *lds);
     ...
     */
    int scon_output_reopen(int output_id, scon_output_stream_t *lds);
     ...
     * the scon_output subsystem to intercept and discard any output sent
     * to the stream via SCON_OUTPUT() or scon_output() until the output
     * is re-enabled.
     */
    bool scon_output_switch(int output_id, bool enable);
     ...
     */
    void scon_output_reopen_all(void);
     ...
     */
    void scon_output_close(int output_id);

    /**
     * Main function to send output to a stream.
     *
     * @param output_id Stream id returned from scon_output_open().
     ...
     * scon_output_verbose() and scon_output_error()).
     *
     ...
     * session directory does not exist when scon_output() is invoked,
     * the output will be discarded!  Once the session directory is
     * created, scon_output() will automatically create the file and
     * writing to it.
     */
    void scon_output(int output_id, const char *format, ...) __scon_attribute_format__(__printf__, 2, 3);
     ...
     * function will effectively invoke scon_output to send the output to
     * the stream.
     *
     * @see scon_output_set_verbosity()
     */
    void scon_output_verbose(int verbose_level, int output_id,
                                           const char *format, ...) __scon_attribute_format__(__printf__, 3, 4);

   /**
    * Same as scon_output_verbose(), but takes a va_list form of varargs.
    */
    void scon_output_vverbose(int verbose_level, int output_id,
                                            const char *format, va_list ap) __scon_attribute_format__(__printf__, 3, 0);
     ...
     * specified by the format parameter.
     *
     * Exactly the same as scon_output_verbose(), except the output it
     ...
     */
    char *scon_output_string(int verbose_level, int output_id,
                                           const char *format, ...) __scon_attribute_format__(__printf__, 3, 4);

   /**
    * Same as scon_output_string, but accepts a va_list form of varargs.
    */
    char *scon_output_vstring(int verbose_level, int output_id,
                                            const char *format, va_list ap) __scon_attribute_format__(__printf__, 3, 0);
     ...
     * will be used for all future invocations of scon_output_verbose().
     */
     ...
     * scon_output_stream_t::lds_want_file is true, the output
     ...
     * after scon_output_set_output_file_info() returns.
     *
     ...
     * to the new directory).  More specifically, the scon_output
     ...
     */
    void scon_output_set_output_file_info(const char *dir,
                                                        const char *prefix,
                                                        char **olddir,
                                                        char **oldprefix);

    /**
     * Same as scon_output_verbose(), but pointer to buffer and size.
     */
    void scon_output_hexdump(int verbose_level, int output_id,
                                           void *ptr, int buflen);
     ...
     * @see scon_output()
     */
#define SCON_OUTPUT(a) scon_output a

    /**
     * Macro for use in sending debugging output to the output
     * streams.  Will be "compiled out" when SCON is configured
     * without --enable-debug.
     *
     * @see scon_output_verbose()
     */
#define SCON_OUTPUT_VERBOSE(a) scon_output_verbose a
#else
    /**
     * Main macro for use in sending debugging output to output streams;
     * will be "compiled out" when SCON is configured without
 ...
 */
SCON_CLASS_DECLARATION(scon_output_stream_t);

END_C_DECLS

#endif /* SCON_OUTPUT_H_ */
