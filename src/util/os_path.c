...
static const char *path_sep = SCON_PATH_SEP;

char *scon_os_path(bool relative, ...)
{
    va_list ap;
    char *element, *path;
    ...
    if (total_length > SCON_PATH_MAX) {  /* path length is too long - reject it */
        return(NULL);
    }
    ...
    va_end(ap);
    return scon_make_filename_os_friendly(path);
}
