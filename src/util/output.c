...
static scon_output_stream_t verbose;
 ...
 * Private functions
 */
static void construct(scon_object_t *stream);
static int do_open(int output_id, scon_output_stream_t * lds);
...
#define SCON_OUTPUT_MAX_STREAMS 64
...
/* global state */
bool scon_output_redirected_to_syslog = false;
int scon_output_redirected_syslog_pri = 0;
...
static output_desc_t info[SCON_OUTPUT_MAX_STREAMS];
...
SCON_CLASS_INSTANCE(scon_output_stream_t, scon_object_t, construct, NULL);
...
{
    int i;
    char hostname[SCON_MAXHOSTNAMELEN];
    char *str;
    ...
    str = getenv("SCON_OUTPUT_STDERR_FD");
    if (NULL != str) {
        default_stderr_fd = atoi(str);
    }
    str = getenv("SCON_OUTPUT_REDIRECT");
    if (NULL != str) {
        if (0 == strcasecmp(str, "syslog")) {
            scon_output_redirected_to_syslog = true;
        }
    }
    str = getenv("SCON_OUTPUT_SYSLOG_PRI");
    if (NULL != str) {
        if (0 == strcasecmp(str, "info")) {
            scon_output_redirected_syslog_pri = LOG_INFO;
        } else if (0 == strcasecmp(str, "error")) {
            scon_output_redirected_syslog_pri = LOG_ERR;
        } else if (0 == strcasecmp(str, "warn")) {
            scon_output_redirected_syslog_pri = LOG_WARNING;
        } else {
            scon_output_redirected_syslog_pri = LOG_ERR;
        }
    } else {
    ...
    str = getenv("SCON_OUTPUT_SYSLOG_IDENT");
    if (NULL != str) {
        redirect_syslog_ident = strdup(str);
    }

    SCON_CONSTRUCT(&verbose, scon_output_stream_t);
    if (scon_output_redirected_to_syslog) {
        verbose.lds_want_syslog = true;
        verbose.lds_syslog_priority = scon_output_redirected_syslog_pri;
        if (NULL != str) {
    ...
    for (i = 0; i < SCON_OUTPUT_MAX_STREAMS; ++i) {
        info[i].ldi_used = false;
        info[i].ldi_enabled = false;

        info[i].ldi_syslog = scon_output_redirected_to_syslog;
    ...
    output_dir = strdup(scon_tmp_directory());

    /* Open the default verbose stream */
    verbose_stream = scon_output_open(&verbose);
    return true;
}
...
#if defined(HAVE_SYSLOG)
        if (i >= SCON_OUTPUT_MAX_STREAMS && syslog_opened) {
            closelog();
        }
#endif
    }
 ...
 */
void scon_output(int output_id, const char *format, ...)
{
    if (output_id >= 0 && output_id < SCON_OUTPUT_MAX_STREAMS) {
        va_list arglist;
 ...
 */
void scon_output_verbose(int level, int output_id, const char *format, ...)
{
    if (output_id >= 0 && output_id < SCON_OUTPUT_MAX_STREAMS &&
 ...
 * Send a message to a stream if the verbose level is high enough
 */
void scon_output_vverbose(int level, int output_id, const char *format,
                          va_list arglist)
{
    if (output_id >= 0 && output_id < SCON_OUTPUT_MAX_STREAMS &&
        info[output_id].ldi_verbose_level >= level) {
 ...
 */
char *scon_output_string(int level, int output_id, const char *format, ...)
{
    int rc;
    char *ret = NULL;
 ...
 * Send a message to a string if the verbose level is high enough
 */
char *scon_output_vstring(int level, int output_id, const char *format,
                          va_list arglist)
{
    int rc;
    char *ret = NULL;
                         ...
                         void *ptr, int buflen)
{
    ...
    if (output_id >= 0 && output_id < SCON_OUTPUT_MAX_STREAMS &&
        ...
        scon_output_verbose(verbose_level, output_id, "dump data at %p %d bytes\n", ptr, buflen);
        for (i = 0; i < buflen; i += 16) {
            ...
            if (ret < 0)
            return;
            scon_output_verbose(verbose_level, output_id, "%s", out_buf);
        }
    }
}
...
void scon_output_finalize(void)
{
    if (initialized) {
        if (verbose_stream != -1) {
            scon_output_close(verbose_stream);
        }
        free(verbose.lds_prefix);
        verbose_stream = -1;
 ...
 * Constructor
 */
static void construct(scon_object_t *obj)
{
    scon_output_stream_t *stream = (scon_output_stream_t*) obj;
...
static int do_open(int output_id, scon_output_stream_t * lds)
{
    ...
    if (scon_output_redirected_to_syslog) {
        ...
        info[i].ldi_syslog_priority = scon_output_redirected_syslog_pri;
...
static int open_file(int i)
{
     ...
     * to open it twice
     */
    for (n=0; n < SCON_OUTPUT_MAX_STREAMS; n++) {
        if (i == n) {
            continue;
        }
        if (!info[n].ldi_used) {
    ...
    if (NULL != output_dir) {
        filename = (char *) malloc(SCON_PATH_MAX);
        if (NULL == filename) {
            return SCON_ERR_OUT_OF_RESOURCE;
        }
        strncpy(filename, output_dir, SCON_PATH_MAX);
        strcat(filename, "/");
...
static int output(int output_id, const char *format, va_list arglist)
{
    ...
    if (output_id >= 0 && output_id < SCON_OUTPUT_MAX_STREAMS &&
        ...
        /* Make the strings */
        if (SCON_SUCCESS != (rc = make_string(&str, ldi, format, arglist))) {
            return rc;
        }
         ...
         * scon_output.h docs about this!). */

        if (ldi->ldi_file) {
            if (ldi->ldi_fd == -1) {
                if (SCON_SUCCESS != open_file(output_id)) {
                    ...
                    snprintf(buffer, BUFSIZ - 1,
                             "[WARNING: %d lines lost because the SCON process session directory did\n not exist when scon_output() was invoked]\n",
                             ldi->ldi_file_num_lines_lost);
