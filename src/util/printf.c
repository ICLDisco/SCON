        ...
        if ('%' == fmt[i] && i + 1 < strlen(fmt)
            ...
            case 's':
                ...
                if (NULL != sarg) {
                    ...
                    scon_output(0, "SCON DEBUG WARNING: Got a NULL argument to scon_vasprintf!\n");
#endif
                    len += 5;
                }
                break;
...
int scon_asprintf(char **ptr, const char *fmt, ...)
{
    int length;
    va_list ap;

    va_start(ap, fmt);
    length = scon_vasprintf(ptr, fmt, ap);
    va_end(ap);
...
int scon_vasprintf(char **ptr, const char *fmt, va_list ap)
{
    int length;
    va_list ap2;
...
#if SCON_HAVE_VA_COPY
    va_copy(ap2, ap);
#elif SCON_HAVE_UNDERSCORE_VA_COPY
    __va_copy(ap2, ap);
#else
...
#if SCON_HAVE_VA_COPY || SCON_HAVE_UNDERSCORE_VA_COPY
    va_end(ap2);
#endif  /* SCON_HAVE_VA_COPY || SCON_HAVE_UNDERSCORE_VA_COPY */
...
int scon_snprintf(char *str, size_t size, const char *fmt, ...)
{
    int length;
    va_list ap;

    va_start(ap, fmt);
    length = scon_vsnprintf(str, size, fmt, ap);
    va_end(ap);
...
int scon_vsnprintf(char *str, size_t size, const char *fmt, va_list ap)
{
    int length;
    char *buf;

    length = scon_vasprintf(&buf, fmt, ap);
    if (length < 0) {
        return length;
    }
...
int main(int argc, char *argv[])
{
    ...
    length = scon_snprintf(NULL, 0, "this is a string %d", 1004);
    printf("length = %d\n", length);

    puts("test of snprintf to an undersize buffer:");
    length = scon_snprintf(a, sizeof(a), "this is a string %d", 1004);
    printf("string = %s\n", a);
    printf("length = %d\n", length);
    printf("strlen = %d\n", (int) strlen(a));

    puts("test of snprintf to an oversize buffer:");
    length = scon_snprintf(b, sizeof(b), "this is a string %d", 1004);
    printf("string = %s\n", b);
    printf("length = %d\n", length);
    printf("strlen = %d\n", (int) strlen(b));

    puts("test of asprintf:");
    length = scon_asprintf(&s, "this is a string %d", 1004);
    printf("string = %s\n", s);
    printf("length = %d\n", length);
    printf("strlen = %d\n", (int) strlen(s));
