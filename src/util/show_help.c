 ...
 * Local functions
 */
static int scon_show_vhelp_internal(const char *filename, const char *topic,
                                    bool want_error_header, va_list arglist);
static int scon_show_help_internal(const char *filename, const char *topic,
                                   bool want_error_header, ...);

scon_show_help_fn_t scon_show_help = scon_show_help_internal;
scon_show_vhelp_fn_t scon_show_vhelp = scon_show_vhelp_internal;


int scon_show_help_init(void)
{
    scon_output_stream_t lds;

    SCON_CONSTRUCT(&lds, scon_output_stream_t);
...
int scon_show_help_finalize(void)
{
    scon_output_close(output_stream);
    output_stream = -1;

    /* destruct the search list */
    if (NULL != search_dirs) {
        scon_argv_free(search_dirs);
        search_dirs = NULL;
    };
...
static int open_file(const char *base, const char *topic)
{
    ...
    if (NULL != search_dirs) {
        ...
        for (i=0; NULL != search_dirs[i]; i++) {
            filename = scon_os_path( false, search_dirs[i], base, NULL );
            scon_show_help_yyin = fopen(filename, "r");
            if (NULL == scon_show_help_yyin) {
                asprintf(&err_msg, "%s: %s", filename, strerror(errno));
                base_len = strlen(base);
                if (4 > base_len || 0 != strcmp(base + base_len - 4, ".txt")) {
                    free(filename);
                    asprintf(&filename, "%s%s%s.txt", search_dirs[i], SCON_PATH_SEP, base);
                    scon_show_help_yyin = fopen(filename, "r");
                }
            }
            free(filename);
    ...
    if (NULL == scon_show_help_yyin) {
        scon_output(output_stream, "%sSorry!  You were supposed to get help about:\n    %s\nBut I couldn't open the help file:\n    %s.  Sorry!\n%s", dash_line, topic, err_msg, dash_line);
        free(err_msg);
    ...
    scon_show_help_init_buffer(scon_show_help_yyin);
...
static int find_topic(const char *base, const char *topic)
{
    ...
    while (1) {
        token = scon_show_help_yylex();
        switch (token) {
        case SCON_SHOW_HELP_PARSE_TOPIC:
            tmp = strdup(scon_show_help_yytext);
            if (NULL == tmp) {
                return SCON_ERR_OUT_OF_RESOURCE;
            }
            tmp[strlen(tmp) - 1] = '\0';
        ...
        case SCON_SHOW_HELP_PARSE_MESSAGE:
            break;

        case SCON_SHOW_HELP_PARSE_DONE:
...
static int read_topic(char ***array)
{
    ...
    while (1) {
        ...
        switch (token) {
        case SCON_SHOW_HELP_PARSE_MESSAGE:
            /* scon_argv_append_nosize does strdup(scon_show_help_yytext) */
            rc = scon_argv_append_nosize(array, scon_show_help_yytext);
            if (rc != SCON_SUCCESS) {
                return rc;
            }
            break;
...
static int load_array(char ***array, const char *filename, const char *topic)
{
    ...
    if (SCON_SUCCESS != (ret = open_file(filename, topic))) {
        return ret;
    }
    ...
    fclose(scon_show_help_yyin);
    scon_show_help_yylex_destroy ();

    if (SCON_SUCCESS != ret) {
        scon_argv_free(*array);
    }

    return ret;
}

char *scon_show_help_vstring(const char *filename, const char *topic,
                             bool want_error_header, va_list arglist)
{
    int rc;
    ...
    /* Load the message */
    if (SCON_SUCCESS != (rc = load_array(&array, filename, topic))) {
        return NULL;
    }
    ...
    scon_argv_free(array);
    return (SCON_SUCCESS == rc) ? output : NULL;
}

char *scon_show_help_string(const char *filename, const char *topic,
                            bool want_error_handler, ...)
{
    ...
    output = scon_show_help_vstring(filename, topic, want_error_handler,
                                    arglist);
    va_end(arglist);
                                    ...
                                    bool want_error_header, va_list arglist)
{
    ...
    output = scon_show_help_vstring(filename, topic, want_error_header,
                                    arglist);

    /* If we got a single string, output it with formatting */
    if (NULL != output) {
        scon_output(output_stream, "%s", output);
        free(output);
    }

    return (NULL == output) ? SCON_ERROR : SCON_SUCCESS;
}
                                   ...
                                   bool want_error_header, ...)
{
    ...
    rc = scon_show_vhelp(filename, topic, want_error_header, arglist);
    va_end(arglist);

    return rc;
}

int scon_show_help_add_dir(const char *directory)
{
    scon_argv_append_nosize(&search_dirs, directory);
    return SCON_SUCCESS;
}
