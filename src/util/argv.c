 ...
 */
scon_status_t scon_argv_append(int *argc, char ***argv, const char *arg)
{
    scon_status_t rc;

    /* add the new element */
    if (SCON_SUCCESS != (rc = scon_argv_append_nosize(argv, arg))) {
        return rc;
    }

    *argc = scon_argv_count(*argv);
...
scon_status_t scon_argv_append_unique_nosize(char ***argv, const char *arg, bool overwrite)
{
     ...
     */
    if (NULL == *argv) {
        return scon_argv_append_nosize(argv, arg);
    }
 ...
 */
static char **scon_argv_split_inter(const char *src_string, int delimiter,
        int include_empty)
{
  char arg[ARGSIZE];
  char **argv = NULL;
  ...
  while (src_string && *src_string) {
    ...
    if (src_string == p) {
      if (include_empty) {
        arg[0] = '\0';
        if (SCON_SUCCESS != scon_argv_append(&argc, &argv, arg))
          return NULL;
      }
    }
    ...
    /* tail argument, add straight from the original string */
      ...
      if (SCON_SUCCESS != scon_argv_append(&argc, &argv, src_string))
    return NULL;
      src_string = p;
      continue;
    }
    ...
    return NULL;
      ...
      if (SCON_SUCCESS != scon_argv_append(&argc, &argv, argtemp)) {
    free(argtemp);
    return NULL;
      }
...
char **scon_argv_split_with_empty(const char *src_string, int delimiter)
{
    return scon_argv_split_inter(src_string, delimiter, 1);
}
 ...
 */
char *scon_argv_join_range(char **argv, size_t start, size_t end, int delimiter)
{
    char **p;
    char *pp;
    char *str;
    ...
    if (NULL == argv || NULL == argv[0] || (int)start > scon_argv_count(argv)) {
        return strdup("");
    }
...
char **scon_argv_copy(char **argv)
{
  ...
  while (NULL != *argv) {
    if (SCON_SUCCESS != scon_argv_append(&dupc, &dupv, *argv)) {
      scon_argv_free(dupv);
      return NULL;
    }
...
scon_status_t scon_argv_delete(int *argc, char ***argv, int start, int num_to_delete)
{
    int i;
    int count;
    int suffix_count;
...
scon_status_t scon_argv_insert(char ***target, int start, char **source)
{
    ...
    target_count = scon_argv_count(*target);
    source_count = scon_argv_count(source);
    if (start > target_count) {
        for (i = 0; i < source_count; ++i) {
            scon_argv_append(&target_count, target, source[i]);
        }
    }
...
scon_status_t scon_argv_insert_element(char ***target, int location, char *source)
{
    ...
    if (location > target_count) {
        scon_argv_append(&target_count, target, source);
        return SCON_SUCCESS;
    }
