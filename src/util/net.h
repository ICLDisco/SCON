...
#ifndef SCON_UTIL_NET_H
#define SCON_UTIL_NET_H
 ...
 * @retval SCON_ERR_TEMP_OUT_OF_RESOURCE Not enough memory for static
 *                        buffer creation
 */
int scon_net_init(void);
 ...
 */
uint32_t scon_net_prefix2netmask(uint32_t prefixlen);
 ...
 *                         false otherwise.
 */
bool scon_net_islocalhost(const struct sockaddr *addr);
 ...
 */
bool scon_net_samenetwork(const struct sockaddr *addr1,
 ...
 */
bool scon_net_addr_isipv4public(const struct sockaddr *addr);
 ...
 */
char* scon_net_get_hostname(const struct sockaddr *addr);
 ...
 */
int scon_net_get_port(const struct sockaddr *addr);
