...
#ifndef SCON_ENVIRON_H
#define SCON_ENVIRON_H
 ...
 * later be freed with a call to scon_argv_free().
 *
 ...
 */
char **scon_environ_merge(char **minor, char **major) __scon_attribute_warn_unused_result__;
 ...
 *
 * @retval SCON_ERR_OUT_OF_RESOURCE If internal malloc() fails.
 * @retval SCON_EXISTS If the name already exists in \em env and
 * \em overwrite is false (and therefore the \em value was not
 * saved in \em env)
 * @retval SCON_SUCESS If the value replaced another value or is
 * appended to \em env.
 *
 ...
 *
 * \code
 *   #include "scon/util/scon_environ.h"
 *   scon_setenv("foo", "bar", true, &environ);
 * \endcode
 *
 * NOTE: If you use the real environ, scon_setenv() will turn
 ...
 * \code
 *   char **my_env = NULL;
 *   scon_setenv("foo", "bar", true, &my_env);
 * \endcode
 */
scon_status_t scon_setenv(const char *name, const char *value,
                              bool overwrite, char ***env) __scon_attribute_nonnull__(1);
 ...
 * @param env The environment to use
 *
 * @retval SCON_ERR_OUT_OF_RESOURCE If an internal malloc fails.
 * @retval SCON_ERR_NOT_FOUND If \em name is not found in \em env.
 * @retval SCON_SUCCESS If \em name is found and successfully deleted.
 *
 * If \em name is found in \em env, the string corresponding to
 * that entry is freed and its entry is eliminated from the array.
 */
scon_status_t scon_unsetenv(const char *name, char ***env) __scon_attribute_nonnull__(1);

/* A consistent way to retrieve the home and tmp directory on all supported
 * platforms.
 */
const char* scon_home_directory( void );
const char* scon_tmp_directory( void );
