 ...
 */
static inline char* scon_find_last_path_separator( const char* filename, size_t n )
{
    char* p = (char*)filename + n;

    /* First skip the latest separators */
    for ( ; p >= filename; p-- ) {
        if( *p != SCON_PATH_SEP[0] )
            break;
    }
...
char *scon_basename(const char *filename)
{
    size_t i;
    char *tmp, *ret = NULL;
    const char sep = SCON_PATH_SEP[0];
    ...
    ret = scon_find_last_path_separator( tmp, strlen(tmp) );
    if (NULL == ret) {
        return tmp;
    }
    ret = strdup(ret + 1);
...
char* scon_dirname(const char* filename)
{
#if defined(HAVE_DIRNAME) || SCON_HAVE_DIRNAME
    char* safe_tmp = strdup(filename), *result;
    result = strdup(dirname(safe_tmp));
    free(safe_tmp);
    return result;
#else
    const char* p = scon_find_last_path_separator(filename, strlen(filename));
...
#endif
                ret[p - filename] = '\0';
                return scon_make_filename_os_friendly(ret);
            }
            break;  /* return the duplicate of "." */
        }
    }
    return strdup(".");
#endif  /* defined(HAVE_DIRNAME) || SCON_HAVE_DIRNAME */
}
