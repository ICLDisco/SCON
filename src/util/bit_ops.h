...
#ifndef SCON_BIT_OPS_H
#define SCON_BIT_OPS_H
 ...
 */
static inline int scon_hibit(int value, int start)
{
    unsigned int mask;
 ...
 * compared to the loop-version (on Intel Nehalem -- with icc-12.1.0 -O2).
 */
static inline int scon_cube_dim(int value)
{
    int dim, size;

#if SCON_C_HAVE_BUILTIN_CLZ
    if (SCON_UNLIKELY (1 >= value)) {
        return 0;
    }
    size = 8 * sizeof(int);
 ...
 * compared to the loop-version (on Intel Nehalem -- with icc-12.1.0 -O2).
 */
static inline int scon_next_poweroftwo_inclusive(int value)
{
    int power2;
