...
#ifndef SCON_UTIL_KEYVAL_PARSE_H
#define SCON_UTIL_KEYVAL_PARSE_H

#include <src/include/scon_config.h>

BEGIN_C_DECLS

extern int scon_util_keyval_parse_lineno;

/**
 * Callback triggered for each key = value pair
 *
 * Callback triggered from scon_util_keyval_parse for each key = value
 ...
 */
typedef void (*scon_keyval_parse_fn_t)(const char *key, const char *value);
 ...
 * scon_util_keyval_parse() will serialize multiple calls.
 */
int scon_util_keyval_parse(const char *filename,
                                         scon_keyval_parse_fn_t callback);

int scon_util_keyval_parse_init(void);

int scon_util_keyval_parse_finalize(void);
