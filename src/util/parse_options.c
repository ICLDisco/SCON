...
void scon_util_parse_range_options(char *inp, char ***output)
{
    ...
    /* split on commas */
    r1 = scon_argv_split(input, ',');
    /* for each resulting element, check for range */
    for (i=0; i < scon_argv_count(r1); i++) {
        r2 = scon_argv_split(r1[i], '-');
        if (1 < scon_argv_count(r2)) {
            ...
            if (-1 == vint) {
                scon_argv_free(*output);
                *output = NULL;
                scon_argv_append_nosize(output, "-1");
                scon_argv_free(r2);
                goto cleanup;
            }
            start = strtol(r2[0], NULL, 10);
        ...
        for (n = start; n <= end; n++) {
            ...
            scon_argv_append_nosize(output, nstr);
        }
        scon_argv_free(r2);
    }
...
cleanup:
    if (bang_option) {
        scon_argv_append_nosize(output, "BANG");
    }
    free(input);
    scon_argv_free(r1);
...
void scon_util_get_ranges(char *inp, char ***startpts, char ***endpts)
{
    ...
    for (i=0; i < scon_argv_count(r1); i++) {
        r2 = scon_argv_split(r1[i], '-');
        if (2 == scon_argv_count(r2)) {
            /* given range - get start and end */
            scon_argv_append_nosize(startpts, r2[0]);
            scon_argv_append_nosize(endpts, r2[1]);
        } else if (1 == scon_argv_count(r2)) {
            /* only one value provided, so it is both the start
             * and the end
             */
            scon_argv_append_nosize(startpts, r2[0]);
            scon_argv_append_nosize(endpts, r2[0]);
        } else {
            /* no idea how to parse this */
            scon_output(0, "%s Unknown parse error on string: %s(%s)",
                        SCON_PRINT_PROC(SCON_PROC_MY_NAME), inp, r1[i]);
        }
        scon_argv_free(r2);
    }
