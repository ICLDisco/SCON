...
bool scon_net_isaddr(const char *name)
{
...
#if SCON_ENABLE_IPV6
static scon_tsd_key_t hostname_tsd_key;
...
get_hostname_buffer(void)
{
    ...
    ret = scon_tsd_getspecific(hostname_tsd_key, &buffer);
    if (SCON_SUCCESS != ret) return NULL;

    if (NULL == buffer) {
        buffer = (void*) malloc((NI_MAXHOST + 1) * sizeof(char));
        ret = scon_tsd_setspecific(hostname_tsd_key, buffer);
    }
...
scon_net_init(void)
{
    ...
    args = scon_argv_split( scon_net_private_ipv4, ';' );
    if( NULL != args ) {
        count = scon_argv_count(args);
        private_ipv4 = (private_ipv4_t*)malloc( (count + 1) * sizeof(private_ipv4_t));
        if( NULL == private_ipv4 ) {
            scon_output(0, "Unable to allocate memory for the private addresses array" );
            scon_argv_free(args);
            goto do_local_init;
        }
        for( i = 0; i < count; i++ ) {
            ...
            if( (a > 255) || (b > 255) || (c > 255) ||
                (d > 255) || (bits > 32) ) {
                if (0 == found_bad) {
                    scon_show_help("help-scon-util.txt",
 ...
 do_local_init:
#if SCON_ENABLE_IPV6
    return scon_tsd_key_create(&hostname_tsd_key, hostname_cleanup);
#else
    return SCON_SUCCESS;
#endif
}
...
uint32_t
scon_net_prefix2netmask(uint32_t prefixlen)
{
...
bool
scon_net_islocalhost(const struct sockaddr *addr)
{
    ...
    default:
        scon_output(0, "unhandled sa_family %d passed to scon_net_islocalhost",
                    addr->sa_family);
        return false;
        break;
    }
...
bool
scon_net_samenetwork(const struct sockaddr *addr1,
    ...
    case AF_INET:
        {
            ...
            uint32_t netmask = scon_net_prefix2netmask (prefixlen);
...
#endif
    default:
        scon_output(0, "unhandled sa_family %d passed to scon_samenetwork",
                    addr1->sa_family);
    }
 ...
 */
bool
scon_net_addr_isipv4public(const struct sockaddr *addr)
{
    switch (addr->sa_family) {
#if SCON_ENABLE_IPV6
        case AF_INET6:
        ...
        case AF_INET:
            {
                ...
                for( i = 0; private_ipv4[i].addr != 0; i++ ) {
                    if( private_ipv4[i].addr == (inaddr->sin_addr.s_addr &
                                                 scon_net_prefix2netmask(private_ipv4[i].netmask_bits)) )
                        return false;
                }
        ...
        default:
            scon_output (0,
                         "unhandled sa_family %d passed to scon_net_addr_isipv4public\n",
                         addr->sa_family);
    }
...
SCON_EXPORT char*
scon_net_get_hostname(const struct sockaddr *addr)
{
#if SCON_ENABLE_IPV6
    ...
    if (NULL == name) {
        scon_output(0, "scon_sockaddr2str: malloc() failed\n");
        return NULL;
    }
    SCON_DEBUG_ZERO(*name);
    ...
    case AF_INET6:
        ...
        if(NULL == inet_ntop(AF_INET6, &((struct sockaddr_in6*) addr)->sin6_addr,
                             name, NI_MAXHOST)) {
            scon_output(0, "scon_sockaddr2str failed with error code %d", errno);
            free(name);
            return NULL;
        }
        return name;
    ...
    if (error) {
       int err = errno;
       scon_output (0, "scon_sockaddr2str failed:%s (return code %i)\n",
