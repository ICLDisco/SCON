...
#define SCON_MSG_PRI    1
#define SCON_ERROR_PRI    2
...
#define scon_event_evtimer_add(x, tv) scon_event_add((x), (tv))

#define scon_event_evtimer_set(b, x, cb, arg) event_assign((x), (b), -1, 0, (event_callback_fn) (cb), (arg))

#define scon_event_evtimer_del(x) scon_event_del((x))

#define scon_event_evtimer_pending(x, tv) event_pending((x), EV_TIMEOUT, (tv))

#define scon_event_evtimer_initialized(x) event_initialized((x))
