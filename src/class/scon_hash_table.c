...
typedef struct scon_hash_element_t scon_hash_element_t;

struct scon_hash_type_methods_t {
    /* Frees any storage associated with the element
     * The value is not owned by the hash table
     * The key,key_size of pointer keys is
     */
    void        (*elt_destructor)(scon_hash_element_t * elt);
    /* Hash the key of the element -- for growing and adjusting-after-removal */
    uint64_t    (*hash_elt)(scon_hash_element_t * elt);
};

/* interact with the class-like mechanism */

static void scon_hash_table_construct(scon_hash_table_t* ht);
static void scon_hash_table_destruct(scon_hash_table_t* ht);

SCON_CLASS_INSTANCE(
    scon_hash_table_t,
    scon_object_t,
    scon_hash_table_construct,
    scon_hash_table_destruct
);

static void
scon_hash_table_construct(scon_hash_table_t* ht)
{
  ht->ht_table = NULL;
...
static void
scon_hash_table_destruct(scon_hash_table_t* ht)
{
...
static size_t
scon_hash_round_capacity_up(size_t capacity)
{
...
int                             /* SCON_ return code */
scon_hash_table_init2(scon_hash_table_t* ht, size_t estimated_max_size,
                      ...
                      int growth_numer, int growth_denom)
{
    ...
    size_t capacity = scon_hash_round_capacity_up(est_capacity);
    ht->ht_table = (scon_hash_element_t*) calloc(capacity, sizeof(scon_hash_element_t));
    if (NULL == ht->ht_table) {
...
int                             /* SCON_ return code */
scon_hash_table_init(scon_hash_table_t* ht, size_t table_size)
{
    /* default to density of 1/2 and growth of 2/1 */
    return scon_hash_table_init2(ht, table_size, 1, 2, 2, 1);
}

int                             /* SCON_ return code */
scon_hash_table_remove_all(scon_hash_table_t* ht)
{
    size_t ii;
    for (ii = 0; ii < ht->ht_capacity; ii += 1) {
        scon_hash_element_t * elt = &ht->ht_table[ii];
...
static int                      /* SCON_ return code */
scon_hash_grow(scon_hash_table_t * ht)
{
    size_t jj, ii;
    scon_hash_element_t* old_table;
    scon_hash_element_t* new_table;
    size_t old_capacity;
    size_t new_capacity;
    ...
    new_capacity = scon_hash_round_capacity_up(new_capacity);

    new_table    = (scon_hash_element_t*) calloc(new_capacity, sizeof(new_table[0]));
    if (NULL == new_table) {
        return SCON_ERR_OUT_OF_RESOURCE;
    }
    ...
    for (jj = 0; jj < old_capacity; jj += 1) {
        scon_hash_element_t * old_elt;
        scon_hash_element_t * new_elt;
...
static int                      /* SCON_ return code */
scon_hash_table_remove_elt_at(scon_hash_table_t * ht, size_t ii)
{
    size_t jj, capacity = ht->ht_capacity;
    scon_hash_element_t* elts = ht->ht_table;
...
static uint64_t
scon_hash_hash_elt_uint32(scon_hash_element_t * elt)
{
  return elt->key.u32;
}

static const struct scon_hash_type_methods_t
scon_hash_type_methods_uint32 = {
    NULL,
    scon_hash_hash_elt_uint32
};

int                             /* SCON_ return code */
scon_hash_table_get_value_uint32(scon_hash_table_t* ht, uint32_t key, void * *value)
{
    ...
    if(capacity == 0) {
        scon_output(0, "scon_hash_table_get_value_uint32:"
                    "scon_hash_table_init() has not been called");
        return SCON_ERROR;
    }
    if (NULL != ht->ht_type_methods &&
        &scon_hash_type_methods_uint32 != ht->ht_type_methods) {
        scon_output(0, "scon_hash_table_get_value_uint32:"
    ...
    ht->ht_type_methods = &scon_hash_type_methods_uint32;
...
int                             /* SCON_ return code */
scon_hash_table_set_value_uint32(scon_hash_table_t * ht, uint32_t key, void * value)
{
    int rc;
    size_t ii, capacity = ht->ht_capacity;
    scon_hash_element_t * elt;
...
#if SCON_ENABLE_DEBUG
    if(capacity == 0) {
        scon_output(0, "scon_hash_table_set_value_uint32:"
                   "scon_hash_table_init() has not been called");
    ...
    for (ii = key%capacity; ; ii += 1) {
        ...
        if (! elt->valid) {
            ...
            if (ht->ht_size >= ht->ht_growth_trigger) {
                if (SCON_SUCCESS != (rc = scon_hash_grow(ht))) {
                    return rc;
                }
            }
            return SCON_SUCCESS;
...
int
scon_hash_table_remove_value_uint32(scon_hash_table_t * ht, uint32_t key)
{
    size_t ii, capacity = ht->ht_capacity;
    ...
    if (NULL != ht->ht_type_methods &&
        &scon_hash_type_methods_uint32 != ht->ht_type_methods) {
        scon_output(0, "scon_hash_table_remove_value_uint32:"
                    "hash table is for a different key type");
            return SCON_ERROR;
    }
#endif
    ...
    for (ii = key%capacity; ; ii += 1) {
        ...
        if (! elt->valid) {
            ...
            return scon_hash_table_remove_elt_at(ht, ii);
        } else {
            /* keep looking */
        }
    }
...
static uint64_t
scon_hash_hash_elt_uint64(scon_hash_element_t * elt)
{
  return elt->key.u64;
}

static const struct scon_hash_type_methods_t
scon_hash_type_methods_uint64 = {
    NULL,
    scon_hash_hash_elt_uint64
};

int                             /* SCON_ return code */
scon_hash_table_get_value_uint64(scon_hash_table_t * ht, uint64_t key, void * *value)
{
    size_t ii;
...
#if SCON_ENABLE_DEBUG
    if(capacity == 0) {
        scon_output(0, "scon_hash_table_get_value_uint64:"
                   "scon_hash_table_init() has not been called");
        return SCON_ERROR;
    }
    if (NULL != ht->ht_type_methods &&
        &scon_hash_type_methods_uint64 != ht->ht_type_methods) {
        scon_output(0, "scon_hash_table_get_value_uint64:"
                    "hash table is for a different key type");
            return SCON_ERROR;
    }
...
int                             /* SCON_ return code */
scon_hash_table_set_value_uint64(scon_hash_table_t * ht, uint64_t key, void * value)
{
    int rc;
    size_t ii, capacity = ht->ht_capacity;
    scon_hash_element_t * elt;
...
int                             /* SCON_ return code */
scon_hash_table_remove_value_uint64(scon_hash_table_t * ht, uint64_t key)
{
    size_t ii, capacity = ht->ht_capacity;
    ...
    if (NULL != ht->ht_type_methods &&
        &scon_hash_type_methods_uint64 != ht->ht_type_methods) {
        scon_output(0, "scon_hash_table_remove_value_uint64:"
                    "hash table is for a different key type");
            return SCON_ERROR;
    }
#endif
...
static uint64_t
scon_hash_hash_key_ptr(const void * key, size_t key_size)
{
    uint64_t hash;
...
static void
scon_hash_destruct_elt_ptr(scon_hash_element_t * elt)
{
...
static uint64_t
scon_hash_hash_elt_ptr(scon_hash_element_t * elt)
{
    return scon_hash_hash_key_ptr(elt->key.ptr.key, elt->key.ptr.key_size);
}

static const struct scon_hash_type_methods_t
scon_hash_type_methods_ptr = {
    scon_hash_destruct_elt_ptr,
    scon_hash_hash_elt_ptr
};

int                             /* SCON_ return code */
scon_hash_table_get_value_ptr(scon_hash_table_t * ht,
                              ...
                              void * *value)
{
...
#if SCON_ENABLE_DEBUG
    if(capacity == 0) {
        scon_output(0, "scon_hash_table_get_value_ptr:"
                   "scon_hash_table_init() has not been called");
        return SCON_ERROR;
    }
    if (NULL != ht->ht_type_methods &&
        &scon_hash_type_methods_ptr != ht->ht_type_methods) {
        scon_output(0, "scon_hash_table_get_value_ptr:"
                    "hash table is for a different key type");
            return SCON_ERROR;
    }
    ...
    ht->ht_type_methods = &scon_hash_type_methods_ptr;
    for (ii = scon_hash_hash_key_ptr(key, key_size)%capacity; ; ii += 1) {
        if (ii == capacity) { ii = 0; }
        elt = &ht->ht_table[ii];
        if (! elt->valid) {
            return SCON_ERR_NOT_FOUND;
...
int                             /* SCON_ return code */
scon_hash_table_set_value_ptr(scon_hash_table_t * ht,
                              const void * key, size_t key_size,
                              void * value)
{
    int rc;
...
#if SCON_ENABLE_DEBUG
    if(capacity == 0) {
        scon_output(0, "scon_hash_table_set_value_ptr:"
                   "scon_hash_table_init() has not been called");
        return SCON_ERR_BAD_PARAM;
    }
    if (NULL != ht->ht_type_methods &&
...
int                             /* SCON_ return code */
scon_hash_table_remove_value_ptr(scon_hash_table_t * ht,
                                 const void * key, size_t key_size)
{
    size_t ii, capacity = ht->ht_capacity;
    ...
    if (NULL != ht->ht_type_methods &&
        &scon_hash_type_methods_ptr != ht->ht_type_methods) {
        scon_output(0, "scon_hash_table_remove_value_ptr:"
                    "hash table is for a different key type");
            return SCON_ERROR;
    }
#endif
...
static int                      /* SCON_ return code */
scon_hash_table_get_next_elt(scon_hash_table_t *ht,
                             scon_hash_element_t * prev_elt, /* NULL means find first */
                             scon_hash_element_t * *next_elt)
{
  scon_hash_element_t* elts = ht->ht_table;
  size_t ii, capacity = ht->ht_capacity;

  for (ii = (NULL == prev_elt ? 0 : (prev_elt-elts)+1); ii < capacity; ii += 1) {
    scon_hash_element_t * elt = &elts[ii];
    if (elt->valid) {
      *next_elt = elt;
      return SCON_SUCCESS;
    }
...
int                             /* SCON_ return code */
scon_hash_table_get_first_key_uint32(scon_hash_table_t * ht,
                                     uint32_t *key, void * *value,
                                     void * *node)
{
  return scon_hash_table_get_next_key_uint32(ht, key, value, NULL, node);
}

int                             /* SCON_ return code */
scon_hash_table_get_next_key_uint32(scon_hash_table_t * ht,
                                    uint32_t *key, void * *value,
                                    void * in_node, void * *out_node)
{
  scon_hash_element_t * elt;
  if (SCON_SUCCESS == scon_hash_table_get_next_elt(ht, (scon_hash_element_t *) in_node, &elt)) {
    *key       = elt->key.u32;
    *value     = elt->value;
    *out_node  = elt;
    return SCON_SUCCESS;
...
int                             /* SCON_ return code */
scon_hash_table_get_first_key_ptr(scon_hash_table_t * ht,
                                  void * *key, size_t *key_size, void * *value,
                                  void * *node)
{
  return scon_hash_table_get_next_key_ptr(ht, key, key_size, value, NULL, node);
}

int                             /* SCON_ return code */
scon_hash_table_get_next_key_ptr(scon_hash_table_t * ht,
                                 void * *key, size_t *key_size, void * *value,
                                 void * in_node, void * *out_node)
{
  scon_hash_element_t * elt;
...
int                             /* SCON_ return code */
scon_hash_table_get_first_key_uint64(scon_hash_table_t * ht,
                                     uint64_t *key, void * *value,
                                     void * *node)
{
  return scon_hash_table_get_next_key_uint64(ht, key, value, NULL, node);
}

int                             /* SCON_ return code */
scon_hash_table_get_next_key_uint64(scon_hash_table_t * ht,
                                    uint64_t *key, void * *value,
                                    void * in_node, void * *out_node)
{
  scon_hash_element_t * elt;
