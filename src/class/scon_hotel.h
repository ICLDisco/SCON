 ...
 *   given time (i.e., when an scon timer event expires).
 ...
 *
 * There is an scon_hotel_init() function to create a hotel, but no
...
typedef void (*scon_hotel_eviction_callback_fn_t)(struct scon_hotel_t *hotel,
                                                  int room_num,
                                                  void *occupant);

/* Note that this is an internal data structure; it is not part of the
   public scon_hotel interface.  Public consumers of scon_hotel
...
typedef struct {
    void *occupant;
    scon_event_t eviction_timer_event;
} scon_hotel_room_t;
 ...
 */
int scon_hotel_init(scon_hotel_t *hotel, int num_rooms,
                                  scon_event_base_t *evbase,
 ...
 *
 * @return SCON_SUCCESS if the occupant is successfully checked in,
 * and the room parameter will contain a valid value.
 * @return SCON_ERR_TEMP_OUT_OF_RESOURCE is the hotel is full.  Try
 * again later.
 */
static inline int scon_hotel_checkin(scon_hotel_t *hotel,
                                     void *occupant,
                                     int *room_num)
{
    scon_hotel_room_t *room;

    /* Do we have any rooms available? */
    if (SCON_UNLIKELY(hotel->last_unoccupied_room < 0)) {
...
/**
 * Same as scon_hotel_checkin(), but slightly optimized for when the
 * caller *knows* that there is a room available.
 */
static inline void scon_hotel_checkin_with_res(scon_hotel_t *hotel,
                                     void *occupant,
                                     int *room_num)
{
    scon_hotel_room_t *room;
 ...
 */
static inline void scon_hotel_checkout(scon_hotel_t *hotel, int room_num)
{
    scon_hotel_room_t *room;
    ...
    room = &(hotel->rooms[room_num]);
    if (SCON_LIKELY(NULL != room->occupant)) {
        /* Do not change this logic without also changing the same
           logic in scon_hotel_checkout_and_return_occupant() and
           scon_hotel.c:local_eviction_callback(). */
        room->occupant = NULL;
        if (NULL != hotel->evbase) {
 ...
 */
static inline void scon_hotel_checkout_and_return_occupant(scon_hotel_t *hotel, int room_num, void **occupant)
{
    scon_hotel_room_t *room;
    ...
    room = &(hotel->rooms[room_num]);
    if (SCON_LIKELY(NULL != room->occupant)) {
        scon_output (10, "checking out occupant %p from room num %d", room->occupant, room_num);
        /* Do not change this logic without also changing the same
           logic in scon_hotel_checkout() and
           scon_hotel.c:local_eviction_callback(). */
 ...
 *
 */
static inline bool scon_hotel_is_empty (scon_hotel_t *hotel)
{
 ...
 */
static inline void scon_hotel_knock(scon_hotel_t *hotel, int room_num, void **occupant)
{
    scon_hotel_room_t *room;
    ...
    *occupant = NULL;
    ...
    room = &(hotel->rooms[room_num]);
    if (SCON_LIKELY(NULL != room->occupant)) {
        scon_output (10, "occupant %p in room num %d responded to knock", room->occupant, room_num);
        *occupant = room->occupant;
    }
}
