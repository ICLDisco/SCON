 ...
 * in the scon_bitmap_t struct
 */
#define SIZE_OF_BASE_TYPE  64

static void scon_bitmap_construct(scon_bitmap_t *bm);
static void scon_bitmap_destruct(scon_bitmap_t *bm);

SCON_CLASS_INSTANCE(scon_bitmap_t, scon_object_t,
                   scon_bitmap_construct, scon_bitmap_destruct);


static void
scon_bitmap_construct(scon_bitmap_t *bm)
{
    bm->bitmap = NULL;
    bm->array_size = 0;
    bm->max_size = INT_MAX;
...
static void
scon_bitmap_destruct(scon_bitmap_t *bm)
{
    if (NULL != bm->bitmap) {
        free(bm->bitmap);
        bm->bitmap = NULL;
...
int
scon_bitmap_clear_all_bits(scon_bitmap_t *bm)
{
    if (NULL == bm) {
        return SCON_ERR_BAD_PARAM;
    }
...
int
scon_bitmap_set_all_bits(scon_bitmap_t *bm)
{
    if (NULL == bm) {
        return SCON_ERR_BAD_PARAM;
    }
...
int
scon_bitmap_find_and_set_first_unset_bit(scon_bitmap_t *bm, int *position)
{
    int i = 0;
    ...
    *position = 0;
    ...
    if (i == bm->array_size) {
        ...
        *position = bm->array_size * SIZE_OF_BASE_TYPE;
        return scon_bitmap_set_bit(bm, *position);
    }
...
bool scon_bitmap_are_different(scon_bitmap_t *left, scon_bitmap_t *right)
{
    ...
    if( scon_bitmap_size(left) != scon_bitmap_size(right) ) {
        return true;
    }
...
char * scon_bitmap_get_string(scon_bitmap_t *bitmap)
{
    ...
    for( i = 0; i < (bitmap->array_size * SIZE_OF_BASE_TYPE); ++i) {
        if( scon_bitmap_is_set_bit(bitmap, i) ) {
            bitmap_str[i] = 'X';
        } else {
            bitmap_str[i] = '_';
        }
...
{
    return (len - scon_bitmap_num_set_bits(bm, len));
}
