 ...
 * @file
 *
 * The scon_list_t interface is used to provide a generic
 * doubly-linked list container for SCON.  It was inspired by (but
 * is slightly different than) the Standard Template Library (STL)
 * std::list class.  One notable difference from std::list is that
 * when an scon_list_t is destroyed, all of the scon_list_item_t
 * objects that it contains are orphaned -- they are \em not
 * destroyed.
 *
 * The general idea is that scon_list_item_t objects can be put on an
 * scon_list_t.  Hence, you create a new type that derives from
 * scon_list_item_t; this new type can then be used with scon_list_t
 * containers.
 *
 * NOTE: scon_list_item_t instances can only be on \em one list at a
 * time.  Specifically, if you add an scon_list_item_t to one list,
 ...
 * have been warned.
 *
 * If SCON_ENABLE_DEBUG is true, a bunch of checks occur, including
 * some spot checks for a debugging reference count in an attempt to
 * ensure that an scon_list_item_t is only one *one* list at a time.
 ...
 * threads may [erroneously] modify an scon_list_item_t concurrently.
 *
 ...
 * \internal
 *
 * Struct of an scon_list_item_t
 */
struct scon_list_item_t
{
    scon_object_t super;
    /**< Generic parent class for all SCON objects */
    volatile struct scon_list_item_t *scon_list_next;
    /**< Pointer to next list item */
    volatile struct scon_list_item_t *scon_list_prev;
    /**< Pointer to previous list item */
    int32_t item_free;

#if SCON_ENABLE_DEBUG
    /** Atomic reference count for debugging */
    volatile int32_t scon_list_item_refcount;
    /** The list this item belong to */
    volatile struct scon_list_t* scon_list_item_belong_to;
#endif
};
/**
 * Base type for items that are put in a list (scon_list_t) containers.
 */
typedef struct scon_list_item_t scon_list_item_t;
 ...
 */
#define scon_list_get_next(item) \
    ((item) ? ((scon_list_item_t*) ((scon_list_item_t*)(item))->scon_list_next) : NULL)
 ...
 * @returns The next item in the list
 */
#define scon_list_get_prev(item) \
    ((item) ? ((scon_list_item_t*) ((scon_list_item_t*)(item))->scon_list_prev) : NULL)


/**
 * \internal
 *
 * Struct of an scon_list_t
 */
struct scon_list_t
{
    scon_object_t       super;
    /**< Generic parent class for all SCON objects */
    scon_list_item_t    scon_list_sentinel;
    /**< Head and tail item of the list */
    volatile size_t     scon_list_length;
    /**< Quick reference to the number of items in the list */
};
/**
 * List container type.
 */
typedef struct scon_list_t scon_list_t;

/** Cleanly destruct a list
 *
 * The scon_list_t destructor doesn't release the items on the
 ...
 */
#define SCON_LIST_DESTRUCT(list)                                \
    do {                                                        \
        scon_list_item_t *it;                                   \
        while (NULL != (it = scon_list_remove_first(list))) {   \
            SCON_RELEASE(it);                                    \
        }                                                       \
        SCON_DESTRUCT(list);                                     \
    } while (0)

#define SCON_LIST_RELEASE(list)                                 \
    do {                                                        \
        scon_list_item_t *it;                                   \
        while (NULL != (it = scon_list_remove_first(list))) {   \
            SCON_RELEASE(it);                                    \
 ...
 *
 * This macro provides a simple way to loop over the items in an scon_list_t. It
 * is not safe to call scon_list_remove_item from within the loop.
 *
 * Example Usage:
 *
 * class_foo_t *foo;
 * scon_list_foreach(foo, list, class_foo_t) {
 *    do something;
 * }
 */
#define SCON_LIST_FOREACH(item, list, type)                             \
  for (item = (type *) (list)->scon_list_sentinel.scon_list_next ;      \
       item != (type *) &(list)->scon_list_sentinel ;                   \
       item = (type *) ((scon_list_item_t *) (item))->scon_list_next)
 ...
 * }
 */
#define SCON_LIST_FOREACH_REV(item, list, type)                         \
  for (item = (type *) (list)->scon_list_sentinel.scon_list_prev ;      \
       item != (type *) &(list)->scon_list_sentinel ;                   \
       item = (type *) ((scon_list_item_t *) (item))->scon_list_prev)
 ...
 *
 * This macro provides a simple way to loop over the items in an scon_list_t. It
 * is safe to call scon_list_remove_item(list, item) from within the loop.
 *
 * Example Usage:
 *
 * class_foo_t *foo, *next;
 * scon_list_foreach_safe(foo, next, list, class_foo_t) {
 *    do something;
 *    scon_list_remove_item (list, (scon_list_item_t *) foo);
 * }
 */
#define SCON_LIST_FOREACH_SAFE(item, next, list, type)                  \
  for (item = (type *) (list)->scon_list_sentinel.scon_list_next,       \
         next = (type *) ((scon_list_item_t *) (item))->scon_list_next ;\
       item != (type *) &(list)->scon_list_sentinel ;                   \
       item = next, next = (type *) ((scon_list_item_t *) (item))->scon_list_next)
 ...
 * @param[in] type Type of each list item
 *
 * This macro provides a simple way to loop over the items in an scon_list_t. If
 * is safe to call scon_list_remove_item(list, item) from within the loop.
 *
 * Example Usage:
 *
 ...
 * }
 */
#define SCON_LIST_FOREACH_SAFE_REV(item, prev, list, type)              \
  for (item = (type *) (list)->scon_list_sentinel.scon_list_prev,       \
         prev = (type *) ((scon_list_item_t *) (item))->scon_list_prev ;\
       item != (type *) &(list)->scon_list_sentinel ;                   \
       item = prev, prev = (type *) ((scon_list_item_t *) (item))->scon_list_prev)
 ...
 */
static inline bool scon_list_is_empty(scon_list_t* list)
{
    return (list->scon_list_sentinel.scon_list_next ==
        &(list->scon_list_sentinel) ? true : false);
}
 ...
 * scon_list_get_end() to ensure that the list is not empty.
 *
 * This is an inlined function in compilers that support inlining, so
 * it's usually a cheap operation.
 */
static inline scon_list_item_t* scon_list_get_first(scon_list_t* list)
{
    scon_list_item_t* item = (scon_list_item_t*)list->scon_list_sentinel.scon_list_next;
#if SCON_ENABLE_DEBUG
    /* Spot check: ensure that the first item is only on one list */

    assert(1 == item->scon_list_item_refcount);
    assert( list == item->scon_list_item_belong_to );
#endif
 ...
 * should be compared against the returned value from
 * scon_list_get_begin() to ensure that the list is not empty.
 *
 * This is an inlined function in compilers that support inlining, so
 * it's usually a cheap operation.
 */
static inline scon_list_item_t* scon_list_get_last(scon_list_t* list)
{
    scon_list_item_t* item = (scon_list_item_t *)list->scon_list_sentinel.scon_list_prev;
#if SCON_ENABLE_DEBUG
 ...
 * it's usually a cheap operation.
 */
static inline scon_list_item_t* scon_list_get_begin(scon_list_t* list)
{
    return &(list->scon_list_sentinel);
}
 ...
 * it's usually a cheap operation.
 */
static inline scon_list_item_t* scon_list_get_end(scon_list_t* list)
{
    return &(list->scon_list_sentinel);
}
 ...
 * the future, calling \c scon_list_splice or \c scon_list_join may
 * result in this function recomputing the list size, which would be
 * an O(N) operation.  If \c scon_list_splice or \c scon_list_join is
 * never called on the specified list, this function will always be
 * O(1).
 */
static inline size_t scon_list_get_size(scon_list_t* list)
{
#if SCON_ENABLE_DEBUG && 0
    ...
    for (item = scon_list_get_first(list) ;
         item != scon_list_get_end(list) ;
         item = scon_list_get_next(item)) {
        check_len++;
    }

    if (check_len != list->scon_list_length) {
        fprintf(stderr," Error :: scon_list_get_size - scon_list_length does not match actual list length\n");
        fflush(stderr);
        abort();
    }
#endif

    return list->scon_list_length;
}
 ...
 * caller -- they are responsible for SCON_RELEASE()'ing it.
 *
 * If debugging is enabled (specifically, if --enable-debug was used
 * to configure SCON), this is an O(N) operation because it checks
 * to see if the item is actually in the list first.
 *
 * This is an inlined function in compilers that support inlining, so
 * it's usually a cheap operation.
 */
static inline scon_list_item_t *scon_list_remove_item
  (scon_list_t *list, scon_list_item_t *item)
{
#if SCON_ENABLE_DEBUG
    scon_list_item_t *item_ptr;
    bool found = false;

    /* check to see that the item is in the list */
    for (item_ptr = scon_list_get_first(list);
            item_ptr != scon_list_get_end(list);
            item_ptr = (scon_list_item_t *)(item_ptr->scon_list_next)) {
        if (item_ptr == (scon_list_item_t *) item) {
            found = true;
            break;
        }
    }
    if (!found) {
        fprintf(stderr," Warning :: scon_list_remove_item - the item %p is not on the list %p \n",(void*) item, (void*) list);
        fflush(stderr);
        return (scon_list_item_t *)NULL;
    }
    ...
    item->scon_list_prev->scon_list_next=item->scon_list_next;

    /* reset previous pointer of next element */
    item->scon_list_next->scon_list_prev=item->scon_list_prev;
    ...
    return (scon_list_item_t *)item->scon_list_prev;
}
 ...
 * The scon_list_item_t is not SCON_RETAIN()'ed; it is assumed that
...
#if SCON_ENABLE_DEBUG
#define scon_list_append(l,i) \
_scon_list_append(l,i,__FILE__,__LINE__)
#else
#define scon_list_append(l,i) \
_scon_list_append(l,i)
#endif  /* SCON_ENABLE_DEBUG */

static inline void _scon_list_append(scon_list_t *list, scon_list_item_t *item
#if SCON_ENABLE_DEBUG
                                     , const char* FILE_NAME, int LINENO
#endif  /* SCON_ENABLE_DEBUG */
                                     )
{
    scon_list_item_t* sentinel = &(list->scon_list_sentinel);
#if SCON_ENABLE_DEBUG
  ...
  item->scon_list_prev = sentinel->scon_list_prev;

  /* reset previous pointer on current last element */
  sentinel->scon_list_prev->scon_list_next = item;

  /* reset new element's next pointer */
  item->scon_list_next = sentinel;

  /* reset the list's tail element previous pointer */
  sentinel->scon_list_prev = item;
 ...
 * list.  The scon_list_item_t is not SCON_RETAIN()'ed; it is assumed
 * that "ownership" of the item is passed from the caller to the list.
 *
 * This is an inlined function in compilers that support inlining, so
 * it's usually a cheap operation.
 */
static inline void scon_list_prepend(scon_list_t *list,
                                     scon_list_item_t *item)
{
    scon_list_item_t* sentinel = &(list->scon_list_sentinel);
#if SCON_ENABLE_DEBUG
  ...
  item->scon_list_next = sentinel->scon_list_next;

  /* reset item's previous pointer */
  item->scon_list_prev = sentinel;

  /* reset previous first element's previous poiner */
  sentinel->scon_list_next->scon_list_prev = item;

  /* reset head's next pointer */
  sentinel->scon_list_next = item;
 ...
 * the caller; no calls to SCON_RETAIN() or SCON_RELEASE() are invoked.
 *
 * This is an inlined function in compilers that support inlining, so
 * it's usually a cheap operation.
 */
static inline scon_list_item_t *scon_list_remove_first(scon_list_t *list)
{
  /*  Removes and returns first item on list.
      Caller now owns the item and should release the item
      when caller is done with it.
  */
  volatile scon_list_item_t *item;
  ...
  assert(1 == list->scon_list_sentinel.scon_list_next->scon_list_item_refcount);
#endif
  ...
  item = list->scon_list_sentinel.scon_list_next;

  /* reset previous pointer of next item on the list */
  item->scon_list_next->scon_list_prev = item->scon_list_prev;

  /* reset the head next pointer */
  list->scon_list_sentinel.scon_list_next = item->scon_list_next;

#if SCON_ENABLE_DEBUG
  assert( list == item->scon_list_item_belong_to );
  item->scon_list_item_belong_to = NULL;
  item->scon_list_prev=(scon_list_item_t *)NULL;
  item->scon_list_next=(scon_list_item_t *)NULL;
 ...
 * it's usually a cheap operation.
 */
static inline scon_list_item_t *scon_list_remove_last(scon_list_t *list)
{
  ...
  */
  ...
  assert(1 == list->scon_list_sentinel.scon_list_prev->scon_list_item_refcount);
#endif
  ...
  /* get item */
  item = list->scon_list_sentinel.scon_list_prev;

  /* reset previous pointer on next to last pointer */
  item->scon_list_prev->scon_list_next = item->scon_list_next;

  /* reset tail's previous pointer */
  list->scon_list_sentinel.scon_list_prev = item->scon_list_prev;

#if SCON_ENABLE_DEBUG
  assert( list == item->scon_list_item_belong_to );
  item->scon_list_next = item->scon_list_prev = (scon_list_item_t *)NULL;
   ...
   */
static inline void scon_list_insert_pos(scon_list_t *list, scon_list_item_t *pos,
                                        scon_list_item_t *item)
{
#if SCON_ENABLE_DEBUG
    ...
    item->scon_list_next = pos;
    item->scon_list_prev = pos->scon_list_prev;

    /* splice into the list */
    pos->scon_list_prev->scon_list_next = item;
    pos->scon_list_prev = item;
   ...
   */
  bool scon_list_insert(scon_list_t *list, scon_list_item_t *item,
                                      long long idx);
     ...
     * but valid after the call.  All pointers to \c scon_list_item_t
     * containers remain valid, including those that point to elements
     * in \c xlist.
     */
    void scon_list_join(scon_list_t *thislist, scon_list_item_t *pos,
                                      scon_list_t *xlist);
     ...
     * be recomputed.
     */
    void scon_list_splice(scon_list_t *thislist, scon_list_item_t *pos,
                                        scon_list_t *xlist, scon_list_item_t *first,
                                        scon_list_item_t *last);

    /**
     * Comparison function for scon_list_sort(), below.
     *
     * @param a Pointer to a pointer to an scon_list_item_t.
     * Explanation below.
     * @param b Pointer to a pointer to an scon_list_item_t.
     * Explanation below.
     ...
     * scon_list_sort().  It is important to understand what
     * scon_list_sort() does before invoking qsort, so go read that
     * documentation first.
     *
     ...
     */
    typedef int (*scon_list_item_compare_fn_t)(scon_list_item_t **a,
                                               scon_list_item_t **b);
     ...
     *    (scon_list_item_t*)'s in an array
     ...
     * a double de-reference to get to the actual scon_list_item_t (or
     * whatever the underlying type is).  See the documentation of
     * scon_list_item_compare_fn_t for an example).
     */
