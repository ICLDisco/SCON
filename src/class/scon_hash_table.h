...
#ifndef SCON_HASH_TABLE_H
#define SCON_HASH_TABLE_H
...
struct scon_hash_table_t
{
    ...
    struct scon_hash_element_t * ht_table;       /**< table of elements (opaque to users) */
    ...
    const struct scon_hash_type_methods_t * ht_type_methods;
};
typedef struct scon_hash_table_t scon_hash_table_t;
...
static inline size_t scon_hash_table_get_size(scon_hash_table_t *ht)
{
    return ht->ht_size;
}
 ...
 *           - SCON_SUCCESS       if key was found
 *           - SCON_ERR_NOT_FOUND if key was not found
 *           - SCON_ERROR         other error
 *
 */
