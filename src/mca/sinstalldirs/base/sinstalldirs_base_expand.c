        ...
        if (NULL != (start_pos = strstr(retval, "${" #fieldname "}"))) { \
            ...
            *start_pos = '\0';                                          \
            ...
            asprintf(&retval, "%s%s%s", tmp,                            \
                     scon_sinstall_dirs.ompiname + destdir_offset,       \
                     end_pos);                                          \
            free(tmp);                                                  \
            changed = true;                                             \
 ...
 */
static char *
scon_sinstall_dirs_expand_internal(const char* input, bool is_setup)
{
    size_t len, i;
    ...
    /* This is subtle, and worth explaining.
       ...
       with the value of the $SCON_DESTDIR environment variable -- if
       it is set.

       We need to handle at least three cases properly (assume that
       configure was invoked with --prefix=/opt/scon and no other
       directory specifications, and SCON_DESTDIR is set to
       /tmp/buildroot):

       1. Individual directories, such as libdir.  These need to be
          prepended with DESTDIR.  I.e., return
          /tmp/buildroot/opt/scon/lib.
       ...
       2. Compiler flags that have ${FIELD} values embedded in them.
          ...
          -DMYFLAG="/tmp/buildroot/opt/scon/share/randomthingy/"
       ...
       Note, too, that this SCON_DESTDIR futzing only needs to occur
       during scon_init().  By the time scon_init() has completed, all
    ...
    if (needs_expand) {
        ...
        do {
            ...
            EXPAND_STRING2(scondatadir, pkgdatadir);
            EXPAND_STRING2(sconlibdir, pkglibdir);
            EXPAND_STRING2(sconincludedir, pkgincludedir);
        } while (changed);
    }

    if (NULL != destdir) {
        char *tmp = retval;
        retval = scon_os_path(false, destdir, tmp, NULL);
        free(tmp);
    }
...
char *
scon_sinstall_dirs_expand(const char* input)
{
    /* We do NOT want SCON_DESTDIR expansion in this case. */
    return scon_sinstall_dirs_expand_internal(input, false);
}


char *
scon_sinstall_dirs_expand_setup(const char* input)
{
    /* We DO want SCON_DESTDIR expansion in this case. */
    return scon_sinstall_dirs_expand_internal(input, true);
}
