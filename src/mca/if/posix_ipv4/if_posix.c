 ...
 */
scon_if_base_component_t mca_if_posix_ipv4_component = {
...
static int if_posix_open(void)
{
    ...
    if ((sd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
        scon_output(0, "scon_ifinit: socket() failed with errno=%d\n",
                    errno);
        return SCON_ERROR;
    }
    ...
    do {
        ...
        if (ioctl(sd, SIOCGIFCONF, &ifconf) < 0) {
            ...
            if (errno != EINVAL && lastlen != 0) {
                scon_output(0, "scon_ifinit: ioctl(SIOCGIFCONF) \
                            failed with errno=%d",
                            errno);
                free(ifconf.ifc_req);
                close(sd);
    ...
    if (!successful_locate) {
        scon_output(0, "scon_ifinit: unable to find network interfaces.");
        close(sd);
    ...
    while (rem > 0) {
        ...
        if (ioctl(sd, SIOCGIFFLAGS, ifr) < 0) {
            scon_output(0, "scon_ifinit: ioctl(SIOCGIFFLAGS) failed with errno=%d", errno);
            continue;
        }
...
#endif
#if 0
        if (!scon_if_retain_loopback && (ifr->ifr_flags & IFF_LOOPBACK) != 0) {
            continue;
        }
#endif
        ...
        if (NULL == intf) {
            scon_output(0, "scon_ifinit: unable to allocated %lu bytes\n", (unsigned long)sizeof(scon_if_t));
            free(ifconf.ifc_req);
            close(sd);
        ...
        if (ioctl(sd, SIOCGIFINDEX, ifr) < 0) {
            scon_output(0,"scon_ifinit: ioctl(SIOCGIFINDEX) failed with errno=%d", errno);
            SCON_RELEASE(intf);
            continue;
        }
        ...
        if (ioctl(sd, SIOCGIFADDR, ifr) < 0) {
            scon_output(0, "scon_ifinit: ioctl(SIOCGIFADDR) failed with errno=%d", errno);
            SCON_RELEASE(intf);
            break;
        }
        if (AF_INET != ifr->ifr_addr.sa_family) {
        ...
        if (ioctl(sd, SIOCGIFNETMASK, ifr) < 0) {
            scon_output(0, "scon_ifinit: ioctl(SIOCGIFNETMASK) failed with errno=%d", errno);
            SCON_RELEASE(intf);
            continue;
        }
        ...
        if (ioctl(sd, SIOCGIFHWADDR, ifr) < 0) {
            scon_output(0, "scon_ifinit: ioctl(SIOCGIFHWADDR) failed with errno=%d", errno);
            break;
        }
        ...
        if (ioctl(sd, SIOCGIFMTU, ifr) < 0) {
            scon_output(0, "scon_ifinit: ioctl(SIOCGIFMTU) failed with errno=%d", errno);
            break;
        }
