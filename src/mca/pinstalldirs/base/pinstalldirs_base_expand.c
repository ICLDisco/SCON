        ...
        if (NULL != (start_pos = strstr(retval, "${" #fieldname "}"))) { \
            ...
            *start_pos = '\0';                                          \
            ...
            asprintf(&retval, "%s%s%s", tmp,                            \
                     scon_pinstall_dirs.ompiname + destdir_offset,       \
                     end_pos);                                          \
            free(tmp);                                                  \
            changed = true;                                             \
 ...
 */
static char *
scon_pinstall_dirs_expand_internal(const char* input, bool is_setup)
{
    size_t len, i;
...
char *
scon_pinstall_dirs_expand(const char* input)
{
    /* We do NOT want SCON_DESTDIR expansion in this case. */
    return scon_pinstall_dirs_expand_internal(input, false);
}


char *
scon_pinstall_dirs_expand_setup(const char* input)
{
    /* We DO want SCON_DESTDIR expansion in this case. */
    return scon_pinstall_dirs_expand_internal(input, true);
}
