 ...
 *
 * Note that there is a second header file (scon_mca_base_vari.h)
 ...
 * itself; it should not be required by any other SCON entities.
 */

#ifndef SCON_MCA_BASE_VAR_H
#define SCON_MCA_BASE_VAR_H
...
#include "src/mca/base/scon_mca_base_var_group.h"
...
typedef enum {
    /** The default value */
    SCON_MCA_BASE_VAR_SOURCE_DEFAULT,
    /** The value came from the command line */
    SCON_MCA_BASE_VAR_SOURCE_COMMAND_LINE,
    /** The value came from the environment */
    SCON_MCA_BASE_VAR_SOURCE_ENV,
    /** The value came from a file */
    SCON_MCA_BASE_VAR_SOURCE_FILE,
    /** The value came a "set" API call */
    SCON_MCA_BASE_VAR_SOURCE_SET,
    /** The value came from the override file */
    SCON_MCA_BASE_VAR_SOURCE_OVERRIDE,

    /** Maximum source type */
    SCON_MCA_BASE_VAR_SOURCE_MAX
} scon_mca_base_var_source_t;
...
typedef enum {
    ...
    SCON_MCA_BASE_VAR_SCOPE_CONSTANT,
    /** Setting the READONLY flag means that the scon_mca_base_var_set()
    ...
    /** The value of this variable must be set to a consistent value
        within a group */
    SCON_MCA_BASE_VAR_SCOPE_GROUP,
    /** The value of this variable must be set to the same value
        within a group */
    SCON_MCA_BASE_VAR_SCOPE_GROUP_EQ,
    /** The value of this variable must be set to a consistent value
        for all processes */
    SCON_MCA_BASE_VAR_SCOPE_ALL,
    /** The value of this variable must be set to the same value
        for all processes */
    SCON_MCA_BASE_VAR_SCOPE_ALL_EQ,
    SCON_MCA_BASE_VAR_SCOPE_MAX
} scon_mca_base_var_scope_t;
...
typedef enum {
    ...
    SCON_INFO_LVL_9,
    SCON_INFO_LVL_MAX
} scon_mca_base_var_info_lvl_t;

typedef enum {
    SCON_MCA_BASE_VAR_SYN_FLAG_DEPRECATED = 0x0001,
    SCON_MCA_BASE_VAR_SYN_FLAG_INTERNAL   = 0x0002
} scon_mca_base_var_syn_flag_t;

typedef enum {
    /** Variable is internal (hidden from *_info/MPIT) */
    SCON_MCA_BASE_VAR_FLAG_INTERNAL     = 0x0001,
    /** Variable will always be the default value. Implies
        !MCA_BASE_VAR_FLAG_SETTABLE */
    SCON_MCA_BASE_VAR_FLAG_DEFAULT_ONLY = 0x0002,
    /** Variable can be set with scon_mca_base_var_set() */
    SCON_MCA_BASE_VAR_FLAG_SETTABLE     = 0x0004,
    /** Variable is deprecated */
    SCON_MCA_BASE_VAR_FLAG_DEPRECATED   = 0x0008,
    /** Variable has been overridden */
    SCON_MCA_BASE_VAR_FLAG_OVERRIDE     = 0x0010,
    /** Variable may not be set from a file */
    SCON_MCA_BASE_VAR_FLAG_ENVIRONMENT_ONLY = 0x0020,
    /** Variable should be deregistered when the group is deregistered
        ...
        scon_mca_base_component_var_register(), but can also be set
        manually when you register a variable with
        scon_mca_base_var_register().  Analogous to the
        MCA_BASE_PVAR_FLAG_IWG. */
    SCON_MCA_BASE_VAR_FLAG_DWG          = 0x0040
} scon_mca_base_var_flag_t;
...
typedef union {
    ...
    /** double value */
    double lfval;
} scon_mca_base_var_storage_t;


/**
 * Entry for holding information about an MCA variable.
 */
struct scon_mca_base_var_t {
    /** Allow this to be an SCON OBJ */
    scon_object_t super;

    /** Variable index. This will remain constant until scon_mca_base_var_finalize()
        is called. */
    int mbv_index;
    /** Group index. This will remain constant until scon_mca_base_var_finalize()
        is called. This variable will be deregistered if the associated group
        is deregistered with scon_mca_base_var_group_deregister() */
    int mbv_group_index;

    /** Info level of this variable */
    scon_mca_base_var_info_lvl_t mbv_info_lvl;

    /** Enum indicating the type of the variable (integer, string, boolean) */
     scon_mca_base_var_type_t mbv_type;
    ...
    /** List of synonym names for this variable.  This *must* be a
        pointer (vs. a plain scon_list_t) because we copy this whole
        struct into a new var for permanent storage
        (scon_vale_array_append_item()), and the internal pointers in
        the scon_list_t will be invalid when that happens.  Hence, we
        simply keep a pointer to an external scon_list_t.  Synonyms
        are uncommon enough that this is not a big performance hit. */
    scon_value_array_t mbv_synonyms;

    /** Variable flags */
    scon_mca_base_var_flag_t  mbv_flags;

    /** Variable scope */
    scon_mca_base_var_scope_t  mbv_scope;

    /** Source of the current value */
    scon_mca_base_var_source_t mbv_source;
    ...
    scon_mca_base_var_enum_t *mbv_enumerator;

    /** Bind value for this variable (0 - none) */
    int mbv_bind;

    /** Storage for this variable */
    scon_mca_base_var_storage_t *mbv_storage;
 ...
 * Convenience typedef.
 */
typedef struct scon_mca_base_var_t scon_mca_base_var_t;
...
/**
 * Object declarayion for scon_mca_base_var_t
 */
SCON_CLASS_DECLARATION(scon_mca_base_var_t);
 ...
 * invoked internally (by scon_mca_base_open()) and is only documented
 ...
 * @retval SCON_ERR_OUT_OF_RESOURCE Upon failure to allocate memory.
 * @retval SCON_ERROR Upon failure to register the variable.
 *
 ...
 * scon_mca_base_var_enum_create(), and describes the valid values of an
 ...
 * scon_mca_base_var_deregister(), scon_mca_base_var_group_deregister(), or
 * scon_mca_base_var_finalize().  {enumerator} should be NULL for
 ...
 * be set using scon_mca_base_var_set_value() (i.e., the MPI_T interface).
 *
 * If MCA_BASE_VAR_FLAG_INTERNAL is set in {flags}, this variable
 * is not shown by default in the output of ompi_info.  That is,
 * this variable is considered internal to the SCON implementation
 ...
 * the description of scon_mca_base_var_scope_t for information about the
 ...
 * successfully.
 */
SCON_EXPORT int scon_mca_base_var_register (const char *project_name, const char *framework_name,
                                const char *component_name, const char *variable_name,
                                const char *description, scon_mca_base_var_type_t type,
                                scon_mca_base_var_enum_t *enumerator, int bind, scon_mca_base_var_flag_t flags,
 ...
 * component.
 *
 * While quite similar to scon_mca_base_var_register(), there is one key
 ...
 * its framework.
 */
SCON_EXPORT int scon_mca_base_component_var_register (const scon_mca_base_component_t *component,
 ...
 * function is equivalent to scon_mca_base_var_register with component_name = "base" and
 * with the MCA_BASE_VAR_FLAG_DWG set. See scon_mca_base_var_register().
 */
SCON_EXPORT int scon_mca_base_framework_var_register (const scon_mca_base_framework_t *framework,
 ...
 * @returns SCON_ERR_BAD_VAR If synonym_for does not reference a valid
 * variable.
 * @returns SCON_ERR_OUT_OF_RESOURCE If memory could not be allocated.
 * @returns SCON_ERROR For all other errors.
 *
 ...
 * scon_mca_base_var_*() API).  If the user sets values for both MCA
 * variable names "B" and "C" (and does *not* set a value for
 * "A"), it is undefined as to which value will be used.
 */
SCON_EXPORT int scon_mca_base_var_register_synonym (int synonym_for, const char *project_name,
 ...
 *
 * @param vari Index returned from scon_mca_base_var_register() or
 * scon_mca_base_var_register_synonym().
 *
 ...
 * scon_mca_base_var_finalize() is called.
 *
 ...
 *
 * @return SCON_ERROR Upon failure.  The contents of value are
 * undefined.
 * @return SCON_SUCCESS Upon success. value (if not NULL) will be filled
 ...
 * the scon_mca_base_var_* interface so the source may be incorrect.
 */
 ...
 * @retval SCON_ERR_PERM If the variable is not settable.
 * @retval SCON_ERR_BAD_PARAM If the variable does not exist or has
 * been deregistered.
 * @retval SCON_ERROR On other error.
 *
 ...
 * settable.
 */
int scon_mca_base_var_set_value (int vari, const void *value, size_t size,
                                 scon_mca_base_var_source_t source,
                                 const char *source_file);
 ...
 *
 * @retval SCON_ERROR If the variable was not found.
 ...
 * scon_mca_base_var_get() and scon_mca_base_var_get_value().
 */
int scon_mca_base_var_find (const char *project_name,
 ...
 *
 * This function will print an scon_show_help() message and return
 * SCON_ERR_BAD_VAR if it finds that the two variables both have
 ...
 * not default values).
 *
 * Note that scon_show_help() allows itself to be hooked, so if this
 * happens after the aggregated scon_show_help() system is
 * initialized, the messages will be aggregated (w00t).
 *
 * @returns SCON_ERR_BAD_VAR if the two variables have sources that
 * are not MCA_BASE_VAR_SOURCE_DEFAULT.
 * @returns SCON_SUCCESS otherwise.
 */
 ...
 *
 * @returns SCON_SUCCESS If the flags are set successfully.
 * @returns SCON_ERR_BAD_PARAM If the variable is not registered.
 * @returns SCON_ERROR Otherwise
 */
int scon_mca_base_var_set_flag(int vari, scon_mca_base_var_flag_t flag,
                               bool set);
 ...
 *
 * @retval SCON_SUCCESS Upon success.
 * @retval scon error code Upon failure.
 *
 ...
 * scon_mca_base_var_deregister() has no impact on the variable count. The count
 * returned is equal to the number of calls to scon_mca_base_var_register with
 ...
 *
 * This function is similar to scon_mca_base_var_dump() except that
 * its output is in terms of an argv-style array of key=value
 * strings, suitable for using in an environment.
 */
int scon_mca_base_var_build_env(char ***env, int *num_env,
                                bool internal);
 ...
 *
 * @returns SCON_SUCCESS This function never fails.
 *
 * This function shuts down the MCA variable repository and frees all
 * associated memory.  No other scon_mca_base_var*() functions can be
...
typedef enum {
    ...
    SCON_MCA_BASE_VAR_DUMP_READABLE = 0,
    /* Dump easily parsable strings */
    SCON_MCA_BASE_VAR_DUMP_PARSABLE = 1,
    /* Dump simple name=value string */
    SCON_MCA_BASE_VAR_DUMP_SIMPLE   = 2
} scon_mca_base_var_dump_type_t;
