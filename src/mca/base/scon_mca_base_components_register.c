 ...
 * Local functions
 */
static int register_components(scon_mca_base_framework_t *framework);
/**
 * Function for finding and opening either all MCA components, or the
 * one that was specifically requested via a MCA parameter.
 */
int scon_mca_base_framework_components_register (scon_mca_base_framework_t *framework,
                                            scon_mca_base_register_flag_t flags)
{
    bool open_dso_components = !(flags & SCON_MCA_BASE_REGISTER_STATIC_ONLY);
    bool ignore_requested = !!(flags & SCON_MCA_BASE_REGISTER_ALL);
    int ret;

    /* Find and load requested components */
    ret = scon_mca_base_component_find(NULL, framework, ignore_requested, open_dso_components);
    if (SCON_SUCCESS != ret) {
        return ret;
    }
...
static int register_components(scon_mca_base_framework_t *framework)
{
    ...
    scon_output_verbose (SCON_MCA_BASE_VERBOSE_COMPONENT, output_id,
                         "scon:mca: base: components_register: registering framework %s components",
                         framework->framework_name);

    /* Traverse the list of found components */

    SCON_LIST_FOREACH_SAFE(cli, next, &framework->framework_components, scon_mca_base_component_list_item_t) {
        component = (scon_mca_base_component_t *)cli->cli_component;

        scon_output_verbose(SCON_MCA_BASE_VERBOSE_COMPONENT, output_id,
                            "scon:mca: base: components_register: found loaded component %s",
        ...
        if (NULL == component->scon_mca_register_component_params) {
            scon_output_verbose (SCON_MCA_BASE_VERBOSE_COMPONENT, output_id,
                                 "scon:mca: base: components_register: "
                                 "component %s has no register or open function",
                                 component->scon_mca_component_name);
            ret = SCON_SUCCESS;
        } else {
            ret = component->scon_mca_register_component_params();
        }
        ...
        if (SCON_SUCCESS != ret) {
            if (SCON_ERR_NOT_AVAILABLE != ret) {
                ...
                if (scon_mca_base_component_show_load_errors) {
                    scon_output_verbose (SCON_MCA_BASE_VERBOSE_ERROR, output_id,
                                         "scon:mca: base: components_register: component %s "
            ...
            scon_list_remove_item (&framework->framework_components, &cli->super);
        ...
        if (NULL != component->scon_mca_register_component_params) {
            scon_output_verbose (SCON_MCA_BASE_VERBOSE_COMPONENT, output_id, "scon:mca: base: components_register: "
                                 "component %s register function successful",
                                 component->scon_mca_component_name);
        }

        /* Register this component's version */
        scon_mca_base_component_var_register (component, "major_version", NULL, SCON_MCA_BASE_VAR_TYPE_INT, NULL,
                                         0, SCON_MCA_BASE_VAR_FLAG_DEFAULT_ONLY | SCON_MCA_BASE_VAR_FLAG_INTERNAL,
                                         SCON_INFO_LVL_9, SCON_MCA_BASE_VAR_SCOPE_CONSTANT,
                                         &component->scon_mca_component_major_version);
        scon_mca_base_component_var_register (component, "minor_version", NULL, SCON_MCA_BASE_VAR_TYPE_INT, NULL,
                                         0, SCON_MCA_BASE_VAR_FLAG_DEFAULT_ONLY | SCON_MCA_BASE_VAR_FLAG_INTERNAL,
                                         SCON_INFO_LVL_9, SCON_MCA_BASE_VAR_SCOPE_CONSTANT,
                                         &component->scon_mca_component_minor_version);
        scon_mca_base_component_var_register (component, "release_version", NULL, SCON_MCA_BASE_VAR_TYPE_INT, NULL,
                                         0, SCON_MCA_BASE_VAR_FLAG_DEFAULT_ONLY | SCON_MCA_BASE_VAR_FLAG_INTERNAL,
                                         SCON_INFO_LVL_9, SCON_MCA_BASE_VAR_SCOPE_CONSTANT,
                                         &component->scon_mca_component_release_version);
    }
