 ...
 */
struct scon_mca_base_open_only_dummy_component_t {
    /** MCA base component */
    scon_mca_base_component_t version;
    /** MCA base data */
    scon_mca_base_component_data_t data;
};
typedef struct scon_mca_base_open_only_dummy_component_t scon_mca_base_open_only_dummy_component_t;
 ...
 *
 * Return one consolidated array of (scon_mca_base_component_t*) pointing to all
...
{
    const scon_mca_base_component_t **static_components = framework->framework_static_components;
    char **requested_component_names = NULL;
    scon_mca_base_component_list_item_t *cli;
    bool include_mode = true;
    int ret;

    scon_output_verbose (SCON_MCA_BASE_VERBOSE_COMPONENT, framework->framework_output,
    ...
    if (static_components) {
        for (int i = 0 ; NULL != static_components[i]; ++i) {
            if ( use_component(include_mode,
                               (const char**)requested_component_names,
                               static_components[i]->scon_mca_component_name) ) {
                cli = SCON_NEW(scon_mca_base_component_list_item_t);
                if (NULL == cli) {
                    ret = SCON_ERR_OUT_OF_RESOURCE;
                    goto component_find_out;
                }
                cli->cli_component = static_components[i];
                scon_list_append(&framework->framework_components, (scon_list_item_t *) cli);
            }
        }
    }
    ...
    if (open_dso_components && !scon_mca_base_component_disable_dlopen) {
        ...
        scon_output_verbose (SCON_MCA_BASE_VERBOSE_INFO, 0,
                            "scon:mca: base: component_find: dso loading for %s MCA components disabled",
...
component_find_out:
    ...
    if (NULL != requested_component_names) {
        scon_argv_free(requested_component_names);
    }
...
int scon_mca_base_components_filter (scon_mca_base_framework_t *framework, uint32_t filter_flags)
{
    ...
    SCON_LIST_FOREACH_SAFE(cli, next, components, scon_mca_base_component_list_item_t) {
        ...
        scon_mca_base_open_only_dummy_component_t *dummy =
            (scon_mca_base_open_only_dummy_component_t *) cli->cli_component;

        can_use = use_component(include_mode, (const char **) requested_component_names,
                                cli->cli_component->scon_mca_component_name);

        if (!can_use || (filter_flags & dummy->data.param_field) != filter_flags) {
            if (can_use && (filter_flags & SCON_MCA_BASE_METADATA_PARAM_CHECKPOINT) &&
                !(SCON_MCA_BASE_METADATA_PARAM_CHECKPOINT & dummy->data.param_field)) {
                scon_output_verbose(SCON_MCA_BASE_VERBOSE_COMPONENT, output_id,
                                    "scon:mca: base: components_filter: "
            ...
            SCON_RELEASE(cli);
        } else if (filter_flags & SCON_MCA_BASE_METADATA_PARAM_CHECKPOINT) {
            scon_output_verbose(SCON_MCA_BASE_VERBOSE_COMPONENT, output_id,
                                "scon:mca: base: components_filter: "
                                ...
                                const char **names, bool include_mode)
{
    ...
    if (NULL != path) {
        ret = scon_mca_base_component_repository_add(path);
        if (SCON_SUCCESS != ret) {
            return;
        }
    }

    ret = scon_mca_base_component_repository_get_components(framework, &dy_components);
    if (SCON_SUCCESS != ret) {
        return;
    }

    /* Iterate through the repository and find components that can be included */
    SCON_LIST_FOREACH(ri, dy_components, scon_mca_base_component_repository_item_t) {
        if (use_component(include_mode, names, ri->ri_name)) {
            scon_mca_base_component_repository_open(framework, ri);
        }
    }
}
...
static int component_find_check (scon_mca_base_framework_t *framework, char **requested_component_names)
{
    ...
    for (int i = 0; NULL != requested_component_names[i]; ++i) {
        ...
        SCON_LIST_FOREACH(cli, components, scon_mca_base_component_list_item_t) {
        ...
        if (!found) {
            ...
            scon_show_help("help-mca-base.txt",
                                        ...
                                        char ***requested_component_names)
{
    ...
    *requested_component_names = scon_argv_split(requested, ',');
