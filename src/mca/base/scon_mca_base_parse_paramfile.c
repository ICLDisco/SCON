...
static scon_list_t * _param_list;
...
int scon_mca_base_parse_paramfile(const char *paramfile, scon_list_t *list)
{
    ...
    return scon_util_keyval_parse(paramfile, save_value);
}

int scon_mca_base_internal_env_store(void)
{
    return scon_util_keyval_save_internal_envars(save_value);
}
...
static void save_value(const char *name, const char *value)
{
    ...
    SCON_LIST_FOREACH(fv, _param_list, scon_mca_base_var_file_value_t) {
    ...
    if (!found) {
        ...
        fv = SCON_NEW(scon_mca_base_var_file_value_t);
        if (NULL == fv) {
            return;
        }

        fv->mbvfv_var = strdup(name);
        scon_list_append(_param_list, &fv->super);
    }

    fv->mbvfv_value = value ? strdup(value) : NULL;
    fv->mbvfv_file  = file_being_read;
    fv->mbvfv_lineno = scon_util_keyval_parse_lineno;
}
