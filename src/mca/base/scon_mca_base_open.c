 ...
 * Public variables
 */
char *scon_mca_base_component_path = NULL;
int scon_mca_base_opened = 0;
char *scon_mca_base_system_default_path = NULL;
char *scon_mca_base_user_default_path = NULL;
bool scon_mca_base_component_show_load_errors = true;
bool scon_mca_base_component_disable_dlopen = false;

static char *scon_mca_base_verbose = NULL;

/*
 * Private functions
 */
static void set_defaults(scon_output_stream_t *lds);
static void parse_verbose(char *e, scon_output_stream_t *lds);
...
int scon_mca_base_open(void)
{
    ...
    if (NULL == scon_mca_base_user_default_path) {
        value = strdup(scon_mca_base_system_default_path);
    } else {
        asprintf(&value, "%s%c%s", scon_mca_base_system_default_path,
                 SCON_ENV_SEP, scon_mca_base_user_default_path);
    }

    scon_mca_base_component_path = value;
    var_id = scon_mca_base_var_register("scon", "mca", "base", "component_path",
                                   "Path where to look for additional components",
                                   SCON_MCA_BASE_VAR_TYPE_STRING, NULL, 0, 0,
                                   SCON_INFO_LVL_9,
                                   SCON_MCA_BASE_VAR_SCOPE_READONLY,
                                   &scon_mca_base_component_path);
    (void) scon_mca_base_var_register_synonym(var_id, "scon", "mca", NULL, "component_path",
                                              SCON_MCA_BASE_VAR_SYN_FLAG_DEPRECATED);
    free(value);

    scon_mca_base_component_show_load_errors = true;
    var_id = scon_mca_base_var_register("scon", "mca", "base", "component_show_load_errors",
                                   "Whether to show errors for components that failed to load or not",
                                   SCON_MCA_BASE_VAR_TYPE_BOOL, NULL, 0, 0,
                                   SCON_INFO_LVL_9,
                                   SCON_MCA_BASE_VAR_SCOPE_READONLY,
                                   &scon_mca_base_component_show_load_errors);
...
/*    scon_mca_base_component_disable_dlopen = false;
    ...
    var_id = scon_mca_base_var_register("scon", "mca", "base", "verbose",
                                   ...
                                   SCON_INFO_LVL_9,
                                   SCON_MCA_BASE_VAR_SCOPE_READONLY,
                                   &scon_mca_base_verbose);
    (void) scon_mca_base_var_register_synonym(var_id, "scon", "mca", NULL, "verbose",
                                              SCON_MCA_BASE_VAR_SYN_FLAG_DEPRECATED);

    memset(&lds, 0, sizeof(lds));
    if (NULL != scon_mca_base_verbose) {
        parse_verbose(scon_mca_base_verbose, &lds);
    } else {
        set_defaults(&lds);
    }
    gethostname(hostname, 64);
    asprintf(&lds.lds_prefix, "[%s:%05d] ", hostname, getpid());
    scon_output_reopen(0, &lds);
    scon_output_verbose (SCON_MCA_BASE_VERBOSE_COMPONENT, 0,
                         "mca: base: opening components at %s", scon_mca_base_component_path);
    free(lds.lds_prefix);

    /* Open up the component repository */

    return scon_mca_base_component_repository_init();
}
