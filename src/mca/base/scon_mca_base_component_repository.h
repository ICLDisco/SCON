...
/**
 * @file scon_mca_base_component_repository.h
 *
 ...
 * internal to the scon/mca/base).  But it's convenient to have all
...
struct scon_mca_base_component_repository_item_t {
    ...
    char ri_type[SCON_MCA_BASE_MAX_TYPE_NAME_LEN + 1];
    char ri_name[SCON_MCA_BASE_MAX_COMPONENT_NAME_LEN + 1];
...
typedef struct scon_mca_base_component_repository_item_t scon_mca_base_component_repository_item_t;

SCON_CLASS_DECLARATION(scon_mca_base_component_repository_item_t);
 ...
 * scon_mca_base_component_path MCA parameter. scon_mca_base_open () is a
 * prerequisite call as it registers the scon_mca_base_component_path parameter.
 */
 ...
 * should call scon_mca_base_component_repository_release() once they are finished with the
 * component.
 *
 * @note all components are automatically unloaded by the
 * scon_mca_base_component_repository_finalize() call.
 */
