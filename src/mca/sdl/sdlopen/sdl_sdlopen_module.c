 ...
 */
static void do_sdlopen(const char *fname, int flags,
...
static int sdlopen_open(const char *fname, bool use_ext, bool private_namespace,
                       scon_sdl_handle_t **handle, char **err_msg)
{
    ...
    *handle = NULL;
    ...
    if (use_ext && NULL != fname) {
        ...
        for (i = 0, ext = mca_sdl_sdlopen_component.filename_suffixes[i];
             NULL != ext;
             ext = mca_sdl_sdlopen_component.filename_suffixes[++i]) {
            char *name;
            ...
            /* Yes, the file exists -- try to dlopen it.  If we can't
               dlopen it, bail. */
            do_sdlopen(name, flags, &local_handle, err_msg);
            free(name);
            break;
        }
    }
    ...
    /* Otherwise, the caller does not want to use filename extensions,
       so just use the single filename that the caller provided */
        ...
        do_sdlopen(fname, flags, &local_handle, err_msg);
    }
    ...
    return (NULL != local_handle) ? SCON_SUCCESS : SCON_ERROR;
}


static int sdlopen_lookup(scon_sdl_handle_t *handle, const char *symbol,
...
static int sdlopen_close(scon_sdl_handle_t *handle)
{
    assert(handle);
 ...
 * on each one.
 */
static int sdlopen_foreachfile(const char *search_path,
                              ...
                              void *data)
{
    ...
    dirs = scon_argv_split(search_path, SCON_ENV_SEP);
    for (int i = 0; NULL != dirs && NULL != dirs[i]; ++i) {

        dp = opendir(dirs[i]);
        if (NULL == dp) {
            ret = SCON_ERR_IN_ERRNO;
            goto error;
        }
        ...
        while (NULL != (de = readdir(dp))) {
            ...
            if (!found) {
                scon_argv_append_nosize(&good_files, abs_name);
            }
            free(abs_name);
        }
        closedir(dp);
 ...
 * Module definition
 */
scon_sdl_base_module_t scon_sdl_sdlopen_module = {
    .open = sdlopen_open,
    .lookup = sdlopen_lookup,
    .close = sdlopen_close,
    .foreachfile = sdlopen_foreachfile
};
