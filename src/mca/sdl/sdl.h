 ...
 * installation).  Meaning: the static_components of the sdl framework
...
typedef struct scon_sdl_handle_t scon_sdl_handle_t;
 ...
 * freed during the call to scon_sdl_base_module_dlclose_fn_t).
 *
 ...
 * change after successive calls to scon_sdl API calls.
 */
typedef int (*scon_sdl_base_module_open_fn_t)
 ...
 * change after successive calls to scon_sdl API calls.
 */
typedef int (*scon_sdl_base_module_lookup_fn_t)
    (scon_sdl_handle_t *handle, const char *symbol, void **ptr, char **err_msg);
 ...
 */
typedef int (*scon_sdl_base_module_close_fn_t)
    (scon_sdl_handle_t *handle);
 ...
 */
typedef int (*scon_sdl_base_module_foreachfile_fn_t)
...
typedef struct scon_sdl_base_component_1_0_0_t scon_sdl_base_component_1_0_0_t;
...
struct scon_sdl_base_module_1_0_0_t {
    ...
    /** Open / close */
    scon_sdl_base_module_open_fn_t           open;
    scon_sdl_base_module_close_fn_t          close;

    /** Lookup a symbol */
    scon_sdl_base_module_lookup_fn_t         lookup;

    /** Iterate looking for files */
    scon_sdl_base_module_foreachfile_fn_t    foreachfile;
};
typedef struct scon_sdl_base_module_1_0_0_t scon_sdl_base_module_1_0_0_t;
