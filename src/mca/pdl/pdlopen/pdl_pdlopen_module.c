                       ...
                       scon_pdl_handle_t **handle, char **err_msg)
{
    ...
    *handle = NULL;
    ...
    if (NULL != local_handle) {
        *handle = calloc(1, sizeof(scon_pdl_handle_t));
...
static int pdlopen_lookup(scon_pdl_handle_t *handle, const char *symbol,
...
static int pdlopen_close(scon_pdl_handle_t *handle)
{
    assert(handle);
 ...
 * Module definition
 */
scon_pdl_base_module_t scon_pdl_pdlopen_module = {
    .open = pdlopen_open,
